function varargout = Perf_Sum_Disp(varargin)
% PERF_SUM_DISP M-file for Perf_Sum_Disp.fig
%      PERF_SUM_DISP, by itself, creates a new PERF_SUM_DISP or raises the existing
%      singleton*.
%
%      H = PERF_SUM_DISP returns the handle to a new PERF_SUM_DISP or the handle to
%      the existing singleton*.
%
%      PERF_SUM_DISP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PERF_SUM_DISP.M with the given input arguments.
%
%      PERF_SUM_DISP('Property','Value',...) creates a new PERF_SUM_DISP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Perf_Sum_Disp_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Perf_Sum_Disp_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Perf_Sum_Disp

% Last Modified by GUIDE v2.5 29-Jun-2008 14:00:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Perf_Sum_Disp_OpeningFcn, ...
                   'gui_OutputFcn',  @Perf_Sum_Disp_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Perf_Sum_Disp is made visible.
function Perf_Sum_Disp_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Perf_Sum_Disp (see VARARGIN)

% Choose default command line output for Perf_Sum_Disp
...
% UIWAIT makes Perf_Sum_Disp wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Perf_Sum_Disp_OutputFcn(hObject, eventdata, handles) 
...
%
  ...
  energy = lcaGetSmart(handles.energy_PV)/0.5794;        % e- energy (GeV)
  wavelength = lcaGetSmart(handles.wavelength_PV);       % FEL wavelength (nm)
  charge(jj) = lcaGetSmart(handles.charge_PV)*1E9*e;     % bunch charge (nC)
  if isnan(charge(jj))
    charge(jj) = 0;
  end
  emitx = lcaGetSmart(handles.emitx_PV);                 % normalized X-emittance [microns]
  emity = lcaGetSmart(handles.emity_PV);                 % normalized Y-emittance [microns]
  bmagx = lcaGetSmart(handles.bmagx_PV);
  bmagy = lcaGetSmart(handles.bmagy_PV);
  BC2Ipk = lcaGetSmart(handles.BC2Ipk_PV);               % peak current (A)
  if BC2Ipk < 10
    BC2Ipk = 1;
  end
  rate = lcaGetSmart(handles.rate_PV);                   % rep, rate (Hz)
  up = charge > low_charge_lim;
  uptime = length(find(up))/length(charge);         % [ ]
  bunchlength = charge(jj)*1E-3*c/sqrt(12)/BC2Ipk;  % rms final bunch length (microns)
  Lgain(jj) = up(jj)*lcaGetSmart(handles.Lgain_PV)/20;   % est. FEL gain length (meters)
  Power(jj) = up(jj)*lcaGetSmart(handles.Power_PV);      % est. FEL output power (GW)
