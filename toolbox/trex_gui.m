function varargout = trex_gui(varargin)
% TREX_GUI MATLAB code for trex_gui.fig
%      TREX_GUI, by itself, creates a new TREX_GUI or raises the existing
%      singleton*.
%
%      H = TREX_GUI returns the handle to a new TREX_GUI or the handle to
%      the existing singleton*.
%
%      TREX_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TREX_GUI.M with the given input arguments.
%
%      TREX_GUI('Property','Value',...) creates a new TREX_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before trex_gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to trex_gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help trex_gui

% Last Modified by GUIDE v2.5 04-Sep-2020 00:12:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @trex_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @trex_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before trex_gui is made visible.
function trex_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to trex_gui (see VARARGIN)

global eDefQuiet;

% Choose default command line output for trex_gui
handles.output       = hObject;

handles = trex_init(handles,hObject); 

% check for signal data
handles.sig_exists = 0;
if exist('trex_sig.mat','file')
    load('trex_sig.mat'); 
...
if exist('trex_sli_erg.mat','file')
    load('trex_sli_erg.mat'); 
    ...
    clear trex_sli_erg.mat sli_erg_mean_avg sli_erg_spread_avg saved_slice_num saved_cut_level erg_corr_ref_DL2 corr_DL2 erg_corr_ref_BC2 corr_BC2 sign_streak;
end
...
handles.fdbkList={'FBCK:FB03:TR04:MODE'};
% Will disable feedback if kick is before...
handles.fdbkDisableBefore = 17; % last und orbit feedback bpm is on cell 28, 16th girder
...
% UIWAIT makes trex_gui wait for user responsreadPVse (see UIRESUME)
% uiwait(handles.trex_gui);
...
function varargout = trex_gui_OutputFcn(hObject, eventdata, handles) 
...
use_bgr = 0;
if exist('trex_bgr.mat','file') == 2;
    load('trex_bgr.mat'); 
    use_bgr = 1;
end

handles.sli_erg_loaded = 0;
if exist('trex_sli_erg.mat','file') == 2 
    load('trex_sli_erg.mat');
...
disp1_tmp        = model_rMatGet('BPMDL1',[],[],'twiss'); disp1 = disp1_tmp(5);
disp2_tmp        = model_rMatGet('BPMDL3',[],[],'twiss'); disp2 = disp2_tmp(5);
try   
    while get(hObject,'Value') == get(hObject,'Max')
        ...
        if get(handles.trex_check_sync,'Value')
            ...
            try
                ...
                %[~,data,readPVs] = evalc('profmon_grabSync(handles,''OTRDMP'',handles.pv_list,1,''doPlot'',0)');
                [data,readPVs] = profmon_grabSyncHST(handles,'OTRDMP',handles.pv_list,1,'doPlot',0);
            catch ex
            ...
            try
                data           = profmon_grab('OTRDMP',0,0);
            catch ex
                warning(['trex_gui glitch with profmon_grab, ' ex.message])
                set(handles.trex_acq_failure,'visible','on');
                drawnow
                pause(0.1)
                continue
            end
            handles.curr1  = lcaGet('BLEN:LI21:265:AIMAX');
            handles.curr2  = lcaGet('BLEN:LI24:886:BIMAX');
            erg1           = lcaGet('BPMS:LTUH:250:X');
            erg2           = lcaGet('BPMS:LTUH:450:X');
            handles.charge = lcaGet('BPMS:DMPH:502:TMIT')*handles.e0;
            xray1          = lcaGet(handles.pv_xray1);
            xray2          = lcaGet(handles.pv_xray2);
            handles.xray   = mean([xray1,xray2]);
            handles.sync   = 0;
        end
       handles.XTCAV_pha_s = lcaGet('TCAV:DMPH:360:PDES');
       handles.XTCAV_amp_s = lcaGet('TCAV:DMPH:360:ADES');
       ...
       mean_erg         = lcaGet('REFS:DMPH:400:EDES')*1E3; 
       handles.mean_erg = mean_erg + mean([erg1/disp1;erg2/disp2])*1E-3*mean_erg;
       ...
       if ~handles.coarse_roi_applied && ~isnan(handles.num_slices) && handles.num_slices > 0 && handles.calibration_applied
           ...
           if get(handles.trex_send_edm,'Value')
                lcaPut('SIOC:SYS0:ML01:AO971',handles.ebeam_duration)
                % This line added back in by TJM for core e spread
                lcaPut('SIOC:SYS0:ML01:AO974',handles.sli_erg_spread(round(numel(handles.sli_erg_spread*handles.mean_erg)/2))*handles.mean_erg)
                % end insert.
           end
           ...
           if get(handles.trex_send_edm,'Value')
               lcaPut('OTRS:DMP1:695:XTCAVWF1',time_axis_curr_edm);
               lcaPut('OTRS:DMP1:695:XTCAVWF2',ebeam_profile');
           end
           ...
           if get(handles.trex_apply_xray_recon,'Value') && handles.sli_erg_loaded 
               ...
               if handles.num_slices == handles.saved_slice_num
                   ...
                   if get(handles.trex_send_edm,'Value')
                       lcaPut('SIOC:SYS0:ML01:AO972',handles.xray_duration)
                       lcaPut('SIOC:SYS0:ML01:AO973',handles.xray/handles.xray_duration*1E3)
                   end
                   ...
                   if get(handles.trex_send_edm,'Value') && ~get(handles.trex_xray_recon_abs,'Value')
                       lcaPut('OTRS:DMP1:695:XTCAVWF3',time_axis_erg_mean_edm);
                       lcaPut('OTRS:DMP1:695:XTCAVWF4',xray_profile');
                   end
       ...
       if get(handles.check_dewake,'value')
           dwfignum = 5185;
           if isempty(findobj('type','figure','name','TREX Dewake-ify'))
               figure(dwfignum)
               set(dwfignum,'name','TREX Dewake-ify')
...
if ~isnan(num_bgr) && num_bgr > 0 && get(hObject,'Value') 
    ...
    try
        [data,ts]      = profmon_grabBG('OTRDMP',num_bgr);
    catch ex
    ...
    data(1).charge = lcaGet('BPMS:DMPH:502:TMIT')*handles.e0;
    data(1).curr1  = lcaGet('BLEN:LI21:265:AIMAX');
    data(1).curr2  = lcaGet('BLEN:LI24:886:BIMAX');
    ...
    data(1).meanErg    = lcaGet('REFS:DMPH:400:EDES')*1E3;
    
    util_dataSave(data,'TREX','BGR_Images',ts(1));
    
    bgr_roi_x = [data(1).roiX+1,data(1).roiX+data(1).roiXN];
    bgr_roi_y = [data(1).roiY+1,data(1).roiY+data(1).roiYN];
                                
    save trex_bgr.mat bgr_avg bgr_roi_x bgr_roi_y -v6;
    ...
    %util_printLog_wComments(f,'TREX','TREX Background Images',elog_comment,[350,200])
    util_printLog(f,'author','TREX','title','TREX Background Images','text',elog_comment);
    close(f)
end
set(hObject,'BackgroundColor',[179 179 179]/255)
...
try
    isIn = lcaGetSmart('OTRS:DMPH:695:PNEUMATIC',1,'double');
    if ~isIn
        warndlg('Asked to get beam images, but screen OTRDMP is not inserted.','TREX GUI','modal');
        return
    end
catch ex
    disp('TREX GUI failed to get dump screen status');
    disp(ex.message)
end
...
if ~isnan(num_bsl) && num_bsl > 0 && get(hObject,'Value') 
    set(hObject,'BackgroundColor',[1 1 0]);pause(0.0);
    try
        [data,readPVs] = profmon_grabSyncHST(handles,'OTRDMP',handles.pv_list,num_bsl,'doPlot',0);
    catch ex
    ...
    data(1).XTCAVPha_s = lcaGet('TCAV:DMPH:360:PDES');
    data(1).XTCAVAmp_s = lcaGet('TCAV:DMPH:360:ADES');
    ...
    erg1               = readPVs(3).val;
    erg2               = readPVs(4).val;
    disp1              = model_rMatGet('BPMDL1',[],[],'twiss'); data(1).disp1 = disp1(5);
    disp2              = model_rMatGet('BPMDL3',[],[],'twiss'); data(1).disp2 = disp2(5);
    ...
    util_dataSave(data,'TREX','BSL_Images',data(1).ts);
    
    bsl(1).bgrFile     = handles.bgr_file;
    ts = data(1).ts;
    
    save trex_bsl.mat bsl charge erg_corr curr_corr sign_streak ts -v6
    handles.bsl_exists = 1;
    ...
    text(-1.5,1-3*shift1,['XTCAV phase (DES): ',num2str(data(1).XTCAVPha_s,'%.1f'),' deg'],'FontSize',12)
    text(-1.5,0-3*shift1,['XTCAV amplitude (DES): ',num2str(data(1).XTCAVAmp_s,'%.1f'),'  MV'],'FontSize',12)
    ...
    %util_printLog_wComments(f,'TREX','TREX Baseline Images',elog_comment,[350,200])
    util_printLog(f,'author','TREX','title','TREX Baseline Images','text',elog_comment);
    close(f)
end
set(hObject,'BackgroundColor',[179 179 179]/255)
...
if ~isnan(num_sig) && num_sig > 0 && get(hObject,'Value') 
    ...
    try
        [data,readPVs] = profmon_grabSyncHST(handles,'OTRDMP',handles.pv_list,num_sig,'doPlot',0);
    catch ex
        warning(['trex_gui glitch with profmon_grabSync, ' ex.message])
        set(hObject,'BackgroundColor',[179 179 179]/255)
        errordlg('profmon_grabSync failed, nothing recorded.','trex_gui');
    ...
    util_dataSave(data,'TREX','SIG_Images',data(1).ts);
    save trex_sig.mat sig readPVs -v6
    handles.sig_exists = 1;
    ...
    if get(handles.trex_suppress_fel,'Value')
        %util_printLog_wComments(f,'TREX','TREX Signal Images with Partial FEL Suppression',elog_comment,[350,200])
        util_printLog(f,'author','TREX','title','TREX Signal Images with Partial FEL Suppression','text',elog_comment);
    else
        %util_printLog_wComments(f,'TREX','TREX Signal Images',elog_comment,[350,200])
        util_printLog(f,'author','TREX','title','TREX Signal Images','text',elog_comment);
    end
    close(f)
end
set(hObject,'BackgroundColor',[179 179 179]/255)
...
% set dispersion at OTRDMP from model
...
if exist('trex_bsl.mat','file') == 2 && ~isnan(handles.num_slices) && handles.num_slices > 0
    set(hObject,'BackgroundColor',[1 1 0]);pause(0.0);
    ...
    save trex_sli_erg.mat sli_erg_mean_avg sli_erg_spread_avg saved_slice_num saved_cut_level erg_corr_ref_DL2 corr_DL2 erg_corr_ref_BC2 corr_BC2 sign_streak -v6
...
if ~isnan(num_jit) && num_jit > 0 && get(hObject,'Value') 
    ...
    [data,readPVs] = profmon_grabSyncHST(handles,'OTRDMP',handles.pv_list,num_jit,'doPlot',0);
    ...
    disp1              = model_rMatGet('BPMDL1',[],[],'twiss'); data(1).disp1 = disp1(5);
    disp2              = model_rMatGet('BPMDL3',[],[],'twiss'); data(1).disp2 = disp2(5);
    erg_jit            = data(1).meanErg + mean([erg1/data(1).disp1;erg2/data(1).disp2])*1E-3*data(1).meanErg;
    ...
    set(BH(2),'Color',handles.colors.TangoScarletRed2,'LineWidth',2);
    set(handles.trex_lps,'YDir','reverse')
    xlabel(handles.trex_lps,'\Delta\phi of XTCAV (deg)')
    ylabel(handles.trex_lps,'\Deltax at OTRDMP (mm)')
    ...
    set(BH(2),'Color',handles.colors.TangoScarletRed2,'LineWidth',2);
    set(handles.trex_xray,'YDir','reverse')
    xlabel(handles.trex_xray,'\Deltat at XTCAV (arb. units.)')
    ylabel(handles.trex_xray,'\Deltax at OTRDMP (mm)')
    ...
    set(handles.trex_ebeam,'YDir','reverse')
    xlabel(handles.trex_ebeam,'\DeltaE at DL2 (MeV)')
    ylabel(handles.trex_ebeam,'\Deltay at OTRDMP (mm)')
    ...
    util_dataSave(data,'TREX','Jitter_Calibration',data(1).ts);   
end
   ...
   handles.dispersion  = handles.dispersion*1E6; 
    ...
    if handles.charge < handles.charge_thres 
        ...
        if ~handles.coarse_roi_applied && ~isnan(handles.num_slices) && handles.num_slices > 0 && handles.calibration_applied
            ...
            handles.sli_erg_loaded  = 0;
            if get(handles.trex_apply_xray_recon,'Value') && exist('trex_sli_erg.mat','file') == 2
                load('trex_sli_erg.mat');
                if  ~isnan(sign_streak) && sign_streak == -1 
                            sli_erg_mean_avg(:,1)   = flipud(sli_erg_mean_avg(:,1)); 
                            sli_erg_mean_avg(:,2)   = flipud(sli_erg_mean_avg(:,2));
...
if handles.init == -1
    ...
    save './trex_dumped/trex_lps.mat' lps_x_axis lps_y_axis lps_int -v6;
    ...
    save './trex_dumped/trex_xray.mat' xray_x_axis1 xray_x_axis2 xray_y_axis1 xray_y_axis2 -v6;
    
    ebeam_x_axis = handles.ebeam_x_axis;
    ebeam_y_axis = handles.ebeam_y_axis;
    
    save './trex_dumped/trex_ebeam.mat' ebeam_x_axis ebeam_y_axis -v6;
...
if handles.beam_on 
  ...
  %  text(-3.75,3.5-shift2,['Streak parameter (init. streak): ',num2str(handles.streak/(1E3*(360*4*2.856E9)*1E-15),'%.2f'),' (',num2str(handles.init_streak/(1E3*(360*4*2.856E9)*1E-15),'%.2f'),') mm/deg'],'FontSize',12)
    ...
    if handles.init ~= -1
        text(-3.75,-1.5-shift4,['XTCAV phase and amplitude (DES): ',num2str(handles.XTCAV_pha_s,'%.1f'),' deg, ',num2str(handles.XTCAV_amp_s,'%.1f'),' MV'],'FontSize',12)
    end   
    ...
    %util_printLog_wComments(f,'TREX','TREX-GUI output',elog_comment,[700,480]);

    if isempty(eventdata)
        util_printLog(f,'author','TREX','title','TREX-GUI output','text',elog_comment);
        close(f)
    end
    if get(handles.check_dewake,'value')
        dwfignum = figure;
        ...
        if isempty(eventdata)
            if isokay
                util_printLog(dwfignum,'author','TREX','title','TREX-GUI Wake Subtraction','text',elog_comment);
            end
            close(dwfignum)
        end
    end
    if isempty(eventdata)
        [ferp,perp] = util_dataSave(data,'TREX','Sample',handles.ts);
        lcaPutSmart('SIOC:SYS0:ML02:CA003',double(int8([perp '/' ferp])));
    end
else
    if handles.init == 1 
        ...
        %util_printLog_wComments(f,'TREX','TREX Baseline Processing',elog_comment,[700,480]) 
        
        if isempty(eventdata)
            util_printLog(f,'author','TREX','title','TREX Baseline Processing','text',elog_comment);
            close(f)
        end
    else
        if  handles.init == 2
            ...
            set(BH(2),'Color',handles.colors.TangoScarletRed2,'LineWidth',2);
            set(gca,'YDir','reverse','FontSize',10)
            xlabel('\Delta\phi of XTCAV (deg)')
            ylabel('\Deltax at OTRDMP (mm)')
            ...
            set(BH(2),'Color',handles.colors.TangoScarletRed2,'LineWidth',2);
            set(gca,'YDir','reverse','FontSize',10)
            xlabel('\Deltat at XTCAV (arb. units.)')
            ylabel('\Deltax at OTRDMP (mm)')
            legend(['Good shots: ', num2str(100*(handles.shot_ratio),'%.0f'),'%'],...
                   ['BAT correction (mm/arb. units.): ', num2str(handles.bat_corr_tmp,'%.3f')],'Location','NorthWest')
            legend('boxoff')
            ...
            set(BH(2),'Color',handles.colors.TangoScarletRed2,'LineWidth',2);
            set(gca,'YDir','reverse','FontSize',10)
            xlabel('\DeltaE at DL2 (MeV)')
            ylabel('\Deltay at OTRDMP (mm)')
            legend(['Good shots: ', num2str(100*(handles.shot_ratio),'%.0f'),'%'],...
                   ['Dispersion D (m): ', num2str(abs(handles.disp_tmp/1E3),'%.3f')],'Location','NorthWest')
            legend('boxoff')
            set(get(gca,'Xlabel'),'FontSize',13)
            ...
            %util_printLog_wComments(f,'TREX','TREX Jitter Calibration',elog_comment,[700,480]) 
            
            if isempty(eventdata)
                util_printLog(f,'author','TREX','title','TREX Jitter Calibration','text',elog_comment);
                close(f)
            end
        end
    end
...
% --- Executes when user attempts to close trex_gui.
function  trex_gui_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to trex_gui (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if get(handles.check_dewake,'value') && isfield(handles,'sli_erg_exists')
    if handles.sli_erg_exists
        fignum = findobj('type','figure','name','TREX Dewake-ify');
        dwfignum = 5185;
        if isempty(fignum)
            fignum = dwfignum;
        else
...
%function isokay = trex_wake_subtraction(handles,dwfignum)
%
% TREX GUI support to plot phase space, less the undulator chamber
...
% Adapted for TREX GUI by T. Maxwell, original code by Y. Ding
...
try
    ...
    titlestr = ['OTRDMP:' datestr(handles.ts,31)];
    title(AX(1),titlestr)
    ...
    title(AX(2),['At U' num2str(und+12,'%02i') ' start'])
...
dy = abs(ys(2)-ys(1));
try
    lcaPutSmart('SIOC:SYS0:ML03:AO601',dx*handles.px2um/handles.streak);
    lcaPutSmart('SIOC:SYS0:ML03:AO602',dy*handles.px2um/handles.dispersion*handles.mean_erg);
catch ex
...
[valList,ts] = lcaGetSyncHST(pvNameList,2800,'CUHBR');
...
Options.RelevantBPM = true(size(static(1).bpmList));
Options.MODEL_TYPE = 'TYPE=EXTANT';
Options.BEAMPATH = 'CU_HXR';
Options.steps = 1;%handles.npoints;
Solutions = undulatorClosedBump('HXR', Options);
