function [ratios, bal, x, y, z, w, mw]= profmon_gaussRatio(data, varargin)
%PROFMON_GAUSSRATIO
...
opts=util_parseOptions(varargin{:},optsdef);
 ...
 %here is henrik's original horz and veritcal
 ...
 if optsdef.diag
    ...
    cw=cw';
     ...
     % subtract background from gaussFit so that util_fwhm call works correctly
     % W. Colocho 11/2017
     wx=util_fwhm(1:length(xcurveCut),xcurveCut - xparametersCut(4));
     wy=util_fwhm(1:length(ycurveCut),ycurveCut - yparametersCut(4));
     wz=util_fwhm(1:length(zcurveCut),zcurveCut - zparametersCut(4));
     ww=util_fwhm(1:length(wcurveCut),wcurveCut - wparametersCut(4));
     mw=mean([wx wy wz ww]);
