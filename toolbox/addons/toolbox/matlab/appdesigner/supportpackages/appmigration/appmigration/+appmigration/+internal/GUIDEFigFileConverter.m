classdef GUIDEFigFileConverter < handle
    ...
    methods
        ...
        function [uifig, callbacks, issues] = convert(obj)
            ...
            isGUIDEApp = isempty(getappdata(obj.GUIDEFigure,'GUIDEOptions'));
            ...
            % Toolbars are migrated so that we can add them
            % programmatically to the StartupFcn. However, need to delete
            % the actual components as they aren't supported in App
            % Designer design environment.
            obj.removeToolbars(uifig);
    ...
    methods (Static)
        ...
        function removeToolbars(fig)
            % Removes any toolbars in the figure including the toolbar
            % children.
            
            toolbars = findall(fig, 'Type', 'uitoolbar');
            delete(toolbars);
        end
