classdef (Sealed, Abstract) AppConversionIssueUtil
    ...
    methods (Static)
        ...
        function code = generateCodeForUitoolbar(issue)
            % Generates code to create a uitoolbar or one of its children
            % (uipushtool or uitoggletool)
            %   Ex: app.uitoolbar1 = uitoolbar(app.figure1);
            
            type = issue.ComponentType;
            codeName = issue.ComponentTag;
            parentCodeName = issue.Value.ParentTag;
            
            code = {...
                sprintf('%% Create %s', codeName);...
                sprintf('app.%s = %s(app.%s);', codeName, type, parentCodeName)...
                };
        end
    end
end
