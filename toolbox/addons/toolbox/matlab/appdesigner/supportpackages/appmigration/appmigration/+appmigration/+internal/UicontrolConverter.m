classdef UicontrolConverter < appmigration.internal.ComponentConverter
    ...
    methods
        ...
        function conversionFuncs = getPropertyConversionFunctions(~)
            ...
            conversionFuncs = {...
                ...
                {'CData'              , @UicontrolConverter.convertCData},...
    ...
    methods (Static)
        ...
        function [pvp, issues] = convertBackgroundColor(guideComponent, prop)
            issues = [];
            pvp = matlab.ui.internal.componentconversion.UicontrolConversionUtils.convertBackgroundColor(guideComponent, prop);
        end
        
        function [pvp, issues] = convertForegroundColor(guideComponent, prop)
            issues = [];
            pvp = matlab.ui.internal.componentconversion.UicontrolConversionUtils.convertForegroundColor(guideComponent, prop);
        end
            ...
            if ~isempty(cdata)
                switch guideComponent.Style
                    case {'pushbutton', 'togglebutton'}
                        % For pushbuttons and togglebuttons, save the cdata
                        % as an image file and set the image path on the
                        % new component.
                        ...
                        pvp = {'Icon', imgName, 'IconAlignment', 'center'};
                    case {'checkbox', 'radiobutton'}
                        % GUIDE users would uses the cdata property on for
                        % these components to display 2D images. This is no
                        % longer supported. Until App Designer has a
                        % decidated image component, report an issue with
                        % the workaround to use an axes and image command.
