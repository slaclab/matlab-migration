function [PVScanValues,MoreValues,PVScanNames,MoreValuesNames]=Scan_gui_calculation_function_delta_disp_scan(INPUT)
...
ADD=linspace(ADDstart,ADDend,NS);
Kvalall=Keff; %linspace(KeffStart,KeffEnd,NS);
PS=PureModeFits.CPLMF;
Correction=ppval(AdditionalDisplacement,ADD);
deltaslot=1;
KVAL=deltagui_Keff2S0(Keff,Harmonic, deltaslot);
%KVALE=deltagui_Keff2S0(KeffEnd,Harmonic, deltaslot);
KValue=sqrt(KVAL)*ones(size(ADD));
...
MoreValues=transpose([Keff*ones(size(Row1Path));DegreeLinPol;Angle;Chirality;HarmonicN]);
