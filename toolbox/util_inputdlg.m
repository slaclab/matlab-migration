function Answer = util_inputdlg ( Prompt, Title, NumLines, DefAns, Resize )
...
% added options:
%  options.minFigWidth  = 175;
%  options.minFigHeight = 100;
%  options.DefOffset    = 5;
%  options.DefBtnWidth  = 53;
%  options.DefBtnHeight = 23;
%
...
%  $Revision: 1.1 $
%  Last modified by Heinz-Dieter Nuhn, SLAC/LCLS on April 15, 2008
%       Added option fields to control the figure window properties.
%
...
if ( isfield ( Options, 'minFigWidth' ) )
    FigWidth = max ( FigWidth, Options.minFigWidth );
end

if ( isfield ( Options, 'minFigHeight' ) )
    FigHeight = max ( FigHeight, Options.minFigHeight );
end
...
%for j = 1 : nProperties
%    fprintf ( 'Trying to set figure property: %s\n', propertyFields { j } );
%    set ( InputFig, propertyFields { j }, Properties.( propertyFields { j } ) );
%end
...
if ( isfield ( Options, 'DefOffset' ) )
    DefOffset = Options.DefOffset;
end

if ( isfield ( Options, 'DefBtnWidth' ) )
    DefBtnWidth = Options.DefBtnWidth;
end

if ( isfield ( Options, 'DefBtnHeight' ) )
    DefBtnHeight = Options.DefBtnHeight;
end
