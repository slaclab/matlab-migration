function list = pl_add_ML00(list, pvnum, desc, egu, prec)
%PL_ADD_ML00
%  LIST = PL_ADD_ML00(LIST, PVNUM, [DESC, EGU, PREC]) adds the Matlab
%  support PV specified by PVNUM to the list of PVs specified by LIST.  The
%  name of the PV is SIOC:SYS0:ML00:AOXXX where XXX is determined by PVNUM,
%  which must be between 0 and 999.
%
%  PL_ADD_ML00 also writes a description, engineering units and numerical
...
%
% Output arguments:
%    LIST:  The modified list, with SIOC:SYS0:ML00:AOXXX added to it.
%
% Usage:
...
pv = ['SIOC:SYS0:ML00:AO' sprintf('%03.0f', abs(pvnum))];
commentpv = ['SIOC:SYS0:ML00:SO0' sprintf('%03.0f', abs(pvnum))];
