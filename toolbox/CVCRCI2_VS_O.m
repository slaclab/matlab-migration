function X = VS_O(QuickVariables,NonSynch,SynchPV,NonStandardSynch,Profiles,ProcessedScalar,AbsoluteCounter,S,SubsetOfElements)
for II=1:length(S(:,1))
    switch(S(II,1))
        case 0
            X{II}=0;
        ...
        case 4
            if(S(II,3)==1)
                X{II} = Profiles{S(II,2)}(SubsetOfElements,:);
            else
                X{II} = Profiles{S(II,2)}(:,:,SubsetOfElements);
            end
        case 5
