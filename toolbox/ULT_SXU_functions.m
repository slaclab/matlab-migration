function fh=ULT_SXU_functions()
    fh.Useg_Init=@Useg_Init; 
    fh.Set_K=@SXU_Set_K;
    fh.Set_K_struct=@SXU_Set_K_struct;
    %fh.Get_K=@SXU_Get_K_from_K;
    %fh.Get_K_alternate=@SXU_Get_K_from_Gap;
    
    fh.Get_K_alternate=@SXU_Get_K_from_Gap;
    fh.Get_K=@SXU_Get_K_from_K;
    
    fh.Get_State_String=@SXU_Get_State_String;
    fh.Set_State=@SXU_Set_State;
    fh.Get_State=@SXU_Get_State;
    
    fh.Set_State_struct=@SXU_Set_State_struct;
    
    fh.Get_Temperature=@SXU_Get_Temperature;
    
    fh.K_to_gap=@K_to_gap;
    fh.gap_to_K=@gap_to_K;
    fh.K_to_gap_dumb=@K_to_gap_dumb;
    
    fh.Get_TaperRange=@SXU_TaperRange;
    fh.LoadSplineFile=@LoadSplineFile;
    
    fh.Move_Out=@SXU_Move_Out;
    fh.Move_In=@SXU_Move_In;
    fh.Move_Out_struct=@SXU_Move_Out_struct;
    fh.Move_In_struct=@SXU_Move_In_struct;
    
    fh.Set_Gap=@SXU_Set_Gap;
    fh.Get_Gap=@SXU_Get_Gap;
    
    fh.LoadKGapConversion=@SXU_LoadKGapConversion;
    ...
    fh.openEDM=@SXU_openEDM;
    
    fh.plotUndulator=@plotUndulator;
end

function Destination=SXU_Move_In_struct(USEG)
...
function SXU_openEDM(CallingObject,Void,USEG,q,c,s) 
%disp(['This will open the EDM screen for undulator ',undulator.PV]);
    PhasPV=regexprep(USEG.PV,'USEG','PHAS');
    PhasPV=regexprep(PhasPV,'50','70');
    PhasPV=[PhasPV,':'];
    CMPV=regexprep(USEG.PV,'USEG','MOVR');
    CMPV=regexprep(CMPV,'50','80');
    CMPV=[CMPV,':'];
    UIOC=['SIOC:UNDS:UC',USEG.Cell_String];
    CIOC=['SIOC:UNDS:MC',USEG.Cell_String];
    CommandString=['! edm -x -m "U=',USEG.PV,':,CELL=',USEG.Cell_String,',PS=',PhasPV,',uioc=',UIOC,',cioc=',CIOC,',I=',CMPV,'" mc_unds_cell_main.edl &'];
    eval(CommandString)
end

function Destination=SXU_Move_Out_struct(USEG)
    Destination.Type=USEG.Type;
    Destination.Cell=USEG.Cell_Number;
    Destination.Gap=USEG.GapOut;
    Destination.GapEnd=USEG.GapOut; 
...
function OUT=SXU_Set_Gap(USEG,GapDes,TaperDes)
    OUT=1;
    if((GapDes<USEG.GapMin) || (GapDes>USEG.GapOut))
        OUT=0;
        return
    end
    UndGapSetRaw ('SXR', USEG.Cell_Number, GapDes, 1);
%     if((GapDes<USEG.GapMin) || (GapDes>USEG.GapOut))
%         OUT=0;
%         return
%     end
...
%    fh=ULT_SXU_functions();
    ...
    USEG.pv.Moving=[USEG.PV,':GapDmov']; % 0 if moving
    USEG.MovingVAL=0; %this says that 1 is moving.
    USEG.pv.TemperatureAct=[USEG.PV,':MeanTemp'];
    USEG.Temperature_Coefficient=lcaGetSmart([USEG.PV,':KTempCoeff']);
    USEG.isMaintenance=0;
    
    USEG.splinefiles.k_vs_gap=['/u1/lcls/epics/ioc/data/sioc-unds-uc',USEG.Cell_String,'/datafiles/sxu_cell',USEG.Cell_String,'_kvsgap_spline.dat'];
    USEG.splinefiles.i1x_vs_gap=['/u1/lcls/epics/ioc/data/sioc-unds-uc',USEG.Cell_String,'/datafiles/sxu_cell',USEG.Cell_String,'_i1xvsgap_spline.dat'];
    USEG.splinefiles.i2x_vs_gap=['/u1/lcls/epics/ioc/data/sioc-unds-uc',USEG.Cell_String,'/datafiles/sxu_cell',USEG.Cell_String,'_i2xvsgap_spline.dat'];
    USEG.splinefiles.i1y_vs_gap=['/u1/lcls/epics/ioc/data/sioc-unds-uc',USEG.Cell_String,'/datafiles/sxu_cell',USEG.Cell_String,'_i1yvsgap_spline.dat'];
    USEG.splinefiles.i2y_vs_gap=['/u1/lcls/epics/ioc/data/sioc-unds-uc',USEG.Cell_String,'/datafiles/sxu_cell',USEG.Cell_String,'_i2yvsgap_spline.dat'];
    USEG.splinefiles.orbitCorrection_vs_gap=['/u1/lcls/matlab/undulator/XYcorrSplineData/GapCorr_S_',regexprep(USEG.PV,':','_'),'.dat'];
    USEG.splinefiles.phase_match_enter_vs_gap=['/u1/lcls/epics/ioc/data/sioc-unds-uc',USEG.Cell_String,'/datafiles/sxu_cell',USEG.Cell_String,'_phasematchentervsgap_spline.dat'];
    USEG.splinefiles.phase_match_exit_vs_gap=['/u1/lcls/epics/ioc/data/sioc-unds-uc',USEG.Cell_String,'/datafiles/sxu_cell',USEG.Cell_String,'_phasematchexitvsgap_spline.dat'];
    ...
    USEG.f.panel={@ULT_SXU_RegularUndulator,@SXU_openEDM};
    USEG.panelNames={'Matlab USEG Panel','Open EDM Screen'};
    %USEG=SXU_LoadKGapConversion(USEG);
...
function Temperature=SXU_Get_Temperature(USEG)
    Temperature=lcaGetSmart(USEG.pv.TemperatureAct);
end

function OUT=SXU_isMoving(USEG)
    VAL=lcaGetSmart(USEG.pv.Moving);
    if(~isnan(VAL))
...
function State_string=SXU_Get_State_String(USEG,ReferencePeriod,K)
    State_string='HOR';
end
...
if(USEG.isMaintenance)
    ...
    %save TEMP
    RegularArea=rectangle('parent',Ax,'position',[USEG.z_ini,Kmin_reg,USEG.z_end-USEG.z_ini,Kmax_reg-Kmin_reg],'FaceColor',[0.7,1,0]);
...
function MovePvs=SXU_Set_K(USEG,Keff,Harmonic,ReferencePeriod,OutputPVList)
...
function Destination=SXU_Set_State_struct(USEG, TargetState,ReferencePeriod, UseRawSettings)
...
function Destination=SXU_Set_K_struct(USEG,Keff,Harmonic,ReferencePeriod)
    %Change K that if passed as Keff into acutal K, for chosen Harmonic
    %number
    Destination.Type=USEG.Type;
    Destination.Cell=USEG.Cell_Number;
...
function [K,Kend]=SXU_Get_K_from_K(USEG,ReferencePeriod,givenPosition)
...
function [K,Kend]=SXU_Get_K_from_Gap(USEG,ReferencePeriod,givenPosition)
if(nargin<3) %it reads the K value from the machine
     Gap = lcaGetSmart(USEG.pv.GapAct);
     Gap_end=lcaGetSmart(USEG.pv.DS_FullGapEncoder);
     %Gap_end=Gap+lcaGetSmart(USEG.pv.TaperAct);
     K = gap_to_K(USEG,Gap);
     Kend=gap_to_K(USEG,Gap_end);
     if(K<USEG.Kout)
        K=0; Kend=0; 
...
function MovePvs=SXU_Set_State(USEG,StateStructure)
    MovePvs=[];
    if(nargin<3)
        if(~OutputPVList) %just do it. 
...
function StateStructure=SXU_Get_State(USEG,ReferencePeriod,givenPosition)
    ...
    if(nargin<3) %it reads the K value from the machine
        ...
        %one can read gap and convert
        Gap = lcaGetSmart(USEG.pv.GapAct);
        %GapTaper=lcaGetSmart(USEG.pv.TaperAct);
        Gap_end=lcaGetSmart(USEG.pv.DS_FullGapEncoder);
...
function KRange=SXU_TaperRange(USEG)
    Gap=lcaGetSmart(USEG.pv.GapAct);
    Gaps=Gap+USEG.TaperRange;
    KRange=gap_to_K(USEG,Gaps);
end

function SXU_Move_Out(USEG)
...
function USEG=SXU_LoadKGapConversion(USEG)
    %This filename is fake will need replacement!!!
    filename=['/u1/lcls/epics/ioc/data/sioc-unds-uc',USEG.Cell_String,'/datafiles/sxu_cell',USEG.Cell_String,'_kvsgap_spline.dat'];
    FID=fopen(filename);
    if(FID==-1)
       disp('File Not Found') 
    end
    ...
    ins=0;USEG.kgap_spline=zeros(60,2);
    while(~feof(FID))
       LINE=fgetl(FID);
       ins=ins+1;
