...
if(Initialize)
    ...
    if(isequal(CurrentDataStructure,StrutturaDatiFull))
        ...
        set(MyHandle.CABET,'string',num2str(petizione.CABET));
        set(MyHandle.UABET,'string',num2str(petizione.UABET));
        set(MyHandle.CAAET,'string',num2str(petizione.CAAET)); 
        set(MyHandle.UAAET,'string',num2str(petizione.UAAET));
        set(MyHandle.axes3,'userdata',[]);
    end
else
    petizione=get(MyHandle.IdatiStanQua,'userdata'); 
...
%                 save TEMPOREX
        ...
        if(petizione.S1 && petizione.S2)
            ...
            MAT=[petizione.UABET,petizione.UAAET;petizione.CABET,petizione.CAAET];
            LH=MAT\[S1;S2];
            %S1
            %S2
            LsH=LH(1)/LH(2);
            Lr=LH(1)/sum(LH)*100;
            Hr=LH(2)/sum(LH)*100;
            CurrentPlot=get(MyHandle.axes3,'userdata');
            cla(MyHandle.axes3,'reset');
            [~,SizeB]=size(CurrentPlot);
            if(isempty(CurrentPlot))
              CurrentPlot(1,1)=Lr;
              CurrentPlot(2,1)=Hr;
            elseif(SizeB>=100)
              CurrentPlot(:,1:99)=CurrentPlot(:,2:100);
              CurrentPlot(1,100)=Lr;
              CurrentPlot(2,100)=Hr;
              SizeB=99;
            else
              CurrentPlot(1,end+1)=Lr;
              CurrentPlot(2,end)=Hr;
            end
            plot(MyHandle.axes3,1:(SizeB+1),CurrentPlot(1,:),'ro:');
            hold(MyHandle.axes3,'on');
            plot(MyHandle.axes3,1:(SizeB+1),CurrentPlot(2,:),'bo:');
            legend(MyHandle.axes3,'Below Edge','Above Edge','location','Southwest');
            set(MyHandle.axes3,'userdata',CurrentPlot);
            set(MyHandle.Risultato,'string',[num2str(Lr,'%3.1f'),' - ',num2str(Hr,'%3.1f')]);
            set(MyHandle.Risultato2,'string',num2str(LsH));
            xlim(MyHandle.axes3,[0,101]);
            ylim(MyHandle.axes3,[0,101]);
        end
    ...
    if(petizione.logbook_and_save || petizione.logbook_only)
       ...
       NuovaFigura=figure;
       copyobj(MyHandle.axes3,NuovaFigura);
       copyobj(MyHandle.axes1,NuovaFigura);
       CHILDREN=get(NuovaFigura,'children');
       set(CHILDREN(1),'position',[12.5 18 75 9.2]);
       title(CHILDREN(1),titlestring)
       set(CHILDREN(2),'position',[12.5 3.5 75 10]);
       xlabel(CHILDREN(2),'Intensities last readings');
       ylabel(CHILDREN(2),'Percent');
       legend(CHILDREN(2),'Below Edge','Above Edge','location','Southwest');
       text
       IV=get(MyHandle.V_SEL,'value');
       IS=get(MyHandle.V_SEL,'string');
       str1=IS{IV};
       ylabel(CHILDREN(1),str1);
       xlabel(CHILDREN(1),['Below Edge - Above Edge:  ' get(MyHandle.Risultato,'string')]);
       try  
           util_printLog(NuovaFigura,'title',['Above and Below Edge Ratio on ',str1]);
       end
