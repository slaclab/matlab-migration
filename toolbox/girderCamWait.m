function status = girderCamWait(segmentList)
%
%   status = girderCamWait(segmentList)
%
...
%
% If a cam becomes "hung" or the emergency stop is set, girderCamWait will
% ignore it so that it doesn't wait forever. 

wait=1;if epicsSimul_status, wait=0.01;end

pause(wait); % make sure status pvs are updated
status = 1;
...
pause(wait); % make sure other pvs update before returning
