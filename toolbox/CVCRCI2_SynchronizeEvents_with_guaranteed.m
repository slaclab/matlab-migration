if(Init_Vars.NumberOfSynchPVs)
    ...
    if(S_Guaranteed(1)==1)
        ...
        PvsCue_PID(S_Guaranteed(3),CO)=NaN;
    end
    IntersectPid=unique(PvsCue_PID(1,:),'stable');
    for TT=2:Init_Vars.NumberOfSynchPVs
        IntersectPid=intersect(IntersectPid,PvsCue_PID(TT,:),'stable');
    end
    ...
    for TT=1:Init_Vars.NumberOfProfiles
       if(CycleVars(TT).Run_Post_Processing)
           if((S_Guaranteed(1)==2) && (FullDataStructure.FromWhichProfileTheScalarMatrixComesFrom(S_Guaranteed(2))==TT ))
               VALORE = ProcessedData(TT).Scalars(S_Guaranteed(3),:);
               CO=(VALORE<ScanSetting.GuaranteedPV_MIN) | (VALORE>ScanSetting.GuaranteedPV_MAX);
               ProcessedData(TT).PulseID(CO)=NaN;
           end
    ...
    ProfCue_PID=bitand(uint32(imag(ProfileCue_TS(:,1:LastValidCueElement))),hex2dec('1FFFF'));
    if(CycleVars(1).Run_Post_Processing)
        if((S_Guaranteed(1)==2) && (FullDataStructure.FromWhichProfileTheScalarMatrixComesFrom(S_Guaranteed(2))==TT ))
               VALORE = ProcessedData(TT).Scalars(S_Guaranteed(3),:);
               CO=(VALORE<ScanSetting.GuaranteedPV_MIN) | (VALORE>ScanSetting.GuaranteedPV_MAX);
               ProcessedData(TT).PulseID(CO)=NaN;
        end
        IntersectPid=unique(ProcessedData(TT).PulseID,'stable');
    else
        ProfCue_PID(1,:)=mod(ProfCue_PID(1,:)+Init_Vars.Profili(1).PulseIDDealy,handles.MAX_Pulse_ID);
        IntersectPid=unique(ProfCue_PID(1,:),'stable');
    end
    for TT=2:Init_Vars.NumberOfProfiles
       if(CycleVars(TT).Run_Post_Processing)
           IntersectPid=intersect(IntersectPid,ProcessedData(TT).PulseID,'stable');
       else
           ProfCue_PID(TT,:)=mod(ProfCue_PID(TT,:)+Init_Vars.Profili(TT).PulseIDDealy,handles.MAX_Pulse_ID);
...
if(NewDataFoundLength)
    if(Init_Vars.NumberOfSynchPVs)
        ...
        SynchProfilePVs(Destination,1) = PvsCue(1,DOVE).';
        TimeStampsMatrix(Destination) = PvsCue_TS(1,DOVE).';
        PulseIDDone=1;TimeStampDone=1;
        for TT=2:Init_Vars.NumberOfSynchPVs
            [~,DOVE,~]=intersect(PvsCue_PID(TT,:),IntersectPid,'stable');
        ...
        for TT=1:Init_Vars.NumberOfProfiles
            if(CycleVars(TT).Run_Post_Processing)
                ...
                if(~TimeStampDone)
                    ...
                    PulseIDMatrix(Destination) = IntersectPid(DoveInPid);
                    TimeStampsMatrix(Destination) = ProfCue_TS(TT,DOVE).';
                    PulseIDDone=1;TimeStampDone=1;
                else
