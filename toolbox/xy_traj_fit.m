function [Xsf,Ysf,p,dp,chisq,Q,Vs] = xy_traj_fit(Xs,dXs,Ys,dYs,Xs0,Ys0,R1s,R3s,fitI)

%   [Xsf,Ysf,p,dp,chisq,Q,Vs] = xy_traj_fit(Xs,dXs,Ys,dYs,Xs0,Ys0,R1s,R3s[,fitI])
%
...
%               fitI:   [Optional,DEF=[1 1 1 1 1]] 1=XPOS, 2=XANG, 3=YPOS, 4=YANG, 5=dE/E (if |eta|>10 mm)
%
...
%if (nXs<2) | (nYs<2)
%  error('Number of X-BPMS and Y-BPMs must be at least 2 each - quitting.')
%end
...
%dXs = dXs';		% force into a row-vector (i.e., one row and N-BPM columns)
%dYs = dYs';		% ...
Xs0 = Xs0(:)';		% ...
Ys0 = Ys0(:)';		% ...
