...
function xtcav_feedback()

appName='xtcav_feedback.m';
...
disp('XTCAV feedback 07/15/20 version 1.3');
lcaSetSeverityWarnLevel(5);  % disables unwanted warnings.

% For magic phase flip
magicCal = 0.905634*.4; % mm*GeV/(degX*MV) 
...
a = a + 1;
front_bpm_x_num = a;
pv{a,1} = ['BPMS:DMPH:325:X', exten];
a = a + 1;
front_bpm_y_num = a;
pv{a,1} = ['BPMS:DMPH:325:Y', exten];
a = a + 1;
front_bpm_t_num = a;
pv{a,1} = ['BPMS:DMPH:325:TMIT',exten];
a = a + 1;
tcav_bpm_x_num = a;
pv{a,1} = ['BPMS:DMPH:502:X', exten];
a = a + 1;
tcav_bpm_y_num = a;
pv{a,1} = ['BPMS:DMPH:502:Y', exten];
a = a + 1;
tcav_bpm_t_num = a;
pv{a,1} = ['BPMS:DMPH:502:TMIT', exten];
a = a + 1;
tcav_phase_num = a;
pv{a,1} = 'TCAV:DMPH:360:PDES';
a = a + 1;
tcav_phase_rb_num = a;
pv{a,1} = 'TCAV:DMPH:360:S_PV';
a = a + 1;
tcav_amplitude_num = a;
pv{a,1}  = 'TCAV:DMPH:360:ADES';
a = a + 1;
tcav_amplitude_rb_num = a;
pv{a,1}  = 'TCAV:DMPH:360:S_AV';
a = a + 1;
tcav_status_num = a;
%pv{a,1} = 'LI24:KLYS:81:SWRD';
pv{a,1} = 'KLYS:DMPH:1:MOD_SET';
...
stby_amp_req_num = a;
pv{a,1} = 'SIOC:SYS0:ML02:AO182'; % ...about that.
a = a + 1;
tcav_rate_num = a;
...
setup_pv(n0, 'tcav feedback counter', ' ', 0, appName);
W = watchdog('SIOC:SYS0:ML01:AO162', 1, 'tcav feedback counter' );
...
pv{a,1} = 'SIOC:SYS0:ML02:AO183';
a = a+1;
stby_countdown_min_num = a;
pv{a,1} = 'SIOC:SYS0:ML02:AO184';
a = a+1;
off_axis_enable_num = a;
pv{a,1} = 'SIOC:SYS0:ML02:AO185';
