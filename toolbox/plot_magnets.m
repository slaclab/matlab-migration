...
ida=strmatch('LCAV',K);        % MAD
ida=[ida;strmatch('RFCW',K)];  % ELEGANT
ida=[ida;strmatch('RFCA',K)];  % MAD or ELEGANT
ida=[ida;strmatch('RFDF',K)];  % ELEGANT
ida=sort(ida);
if (~isempty(ida))
...
idb=strmatch('SBEN',K);        % MAD or ELEGANT
idb=[idb;strmatch('RBEN',K)];  % MAD or ELEGANT
idb=[idb;strmatch('NIBE',K)];  % ELEGANT
idb=[idb;strmatch('CSBE',K)];  % ELEGANT
idb=[idb;strmatch('KSBE',K)];  % ELEGANT
idb=[idb;strmatch('CSRC',K)];  % ELEGANT
idb=sort(idb);
if (~isempty(idb))
...
idq=strmatch('QUAD',K);        % MAD or ELEGANT
idq=[idq;strmatch('KQUA',K)];  % ELEGANT
idq=sort(idq);
if (~isempty(idq))
...
ids=strmatch('SEXT',K);        % MAD or ELEGANT
ids=[ids;strmatch('KSEX',K)];  % ELEGANT
ids=sort(ids);
if (~isempty(ids))
...
ido=strmatch('OCTU',K);        % MAD or ELEGANT
if (~isempty(ido))
...
idr=strmatch('SOLE',K);        % MAD or ELEGANT
if (~isempty(idr))
