function bpmDisp(beampath)

%Calculates correlation coefficient between all bpms and dispersion BPM and
%writes information to waveform PV for different beampaths

% Features:

% Input parameters: 
    % beampath: Name of beamline, accepted names ('CU_HXR',CU_SXR',...)
...
%Declaring PVs, cells with dispersion bpms, and WFs to be used on each beampath
switch beampath
    
    case 'CU_HXR'
        beampath_model = 'BEAMPATH=CU_HXR';
        disp_bpms_cellArray = {'BPMS:LI21:233', 'BPMS:LI24:801', 'BPMS:LTUH:250', 'BPMS:LTUH:450'};
        n=model_nameRegion('BPMS', beampath);% gets a list of all BPMS for a specific beampath
        ...
        PV_unknown = 'SIOC:SYS0:ML01:AO071';
    ...
    case 'CU_SXR'
        beampath_model= 'BEAMPATH=CU_SXR';
         ...
         PV_unknown = 'SIOC:SYS0:ML02:AO177';
...
[rMat, zPos, lEff, twiss, energy] = model_rMatGet(disp_bpms_cellArray,[],{'TYPE=DESIGN',strcat('BEAMPATH=', beampath)}); %#ok<ASGLU>
etaValAtBPM = 1000*twiss(5,:) % Eta in the model is in m, so we set it to mm
 
nAll=[strcat(n,':X');strcat(n,':Y');strcat(n,':TMIT')];
        
%Gathering data for eacccch beampath          
while (1)   
        ...
        try
            ...
            lcaPutSmart(PV_unknown, isRunning);
           ...
           if ~beamOnFirst, pause(5), continue, end
                ...
                for k = 1:6
                    ...
                    try
                        ...
                        if length(ts1) >= nMeas*0.8 % Why 0.8?
                            break
                        end
                        pause(nMeas/beamRate);
                    catch
                        fprintf('%s Failed lcaGetSyncHST(nAll), pausing...',datestr(now));% we need the BSA system for the SC line
                        pause(30)
                    end
          ...
          % Calculating WFs for each beampath
          switch beampath
              case 'CU_HXR'
              ...
              case 'CU_SXR'
                  ...
                  for ii = 1:nBpms % nBpms will change with the beampath
                      ...
                      %pFitX_CLTS_D2(ii,:) = nanToZero(polyfit(bpmCLTS_D2x,x(ii,:),1));
                      %pFitY_CLTS_D2(ii,:) = nanToZero(polyfit(bpmCLTS_D2x,y(ii,:),1));
                  ...
                  %% Look for Burst mode DL2 BPM energy symetry problem
                  %For soft line we will need two symmetricParts for CLTS and
                  
                  %LTUS
                  
                  bpmDl1Mean = 1000 * mean(bpmDl1x);
                  bpmDl2Mean = 1000 * mean(bpmDl2x); %Microns
            ...
            % Writing output to waveform PVs, we need new WF for the soft
            % line. 

                zP = 3000 * ones(1,176); % 176 is nBpms
                zP(1:length(z)) = z; % the real z values are replaced and the remaining ones are unvisible on the EPICs display
                if beamOnBothLTUS, lcaPutSmart(PV_symmetricPartLTUS,symmetricPartLTUS); end % Waveform size is of 166
                if beamOnBothCLTS 
...
%                 
%                 figure(2)
%                 plot(z,bpmDl1_eta_LTUS* pFitY_DL1(:,1)', 'g')
%                 xlabel('z')
%                 ylabel('LTUS dispersion wave Y')
%                 figure(3)
%                 plot(z,bpmDl1_eta_CLTS* pFitX_CLTS_D1(:,1)')
%                 xlabel('z')
%                 ylabel('CLTS dispersion wave X')
%                 figure(4)
%                 plot(z,bpmDl1_eta_CLTS* pFitY_CLTS_D1(:,1)')
%                 xlabel('z')
%                 ylabel('CLTS dispersion wave Y')
