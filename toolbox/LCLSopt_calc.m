function [sz,sd,k,Ipk,dE_E,dt,Eloss,sdsgn] = LCLSopt_calc(sz0,sd0,Eg,Ev,R56v,T566v,phiv,Lv,N,...
                                                         dN_N,dtg,dphiv,lamv,s0v,av);

% [sz,sd,k,Ipk,dE_E,dt,Eloss,sdsgn] = LCLSopt_calc(sz0,sd0,Eg,Ev,R56v,T566v,phiv,Lv,N,...
%                                                 dN_N,dtg,dphiv,lamv,s0v,av);
%
...
for j = 1:Nv						        % loop over all linac-compressor segments
  ...
  dE_E(j)     = dE_E1(j)*Er(j) + (1-Er(j))*...
                (cos(phivr(j) + dphi)/C(j) - 1) + ...
                kw*dN_Nf/(1+dN_Nf)*ds/2;	% relative energy error due to dphase and dN error [ ]
