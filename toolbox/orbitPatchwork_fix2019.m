function varargout = orbitPatchwork_fix2019(varargin)
% ORBITPATCHWORK_FIX2019 MATLAB code for orbitPatchwork_fix2019.fig
%      ORBITPATCHWORK_FIX2019, by itself, creates a new ORBITPATCHWORK_FIX2019 or raises the existing
%      singleton*.
%
%      H = ORBITPATCHWORK_FIX2019 returns the handle to a new ORBITPATCHWORK_FIX2019 or the handle to
%      the existing singleton*.
%
%      ORBITPATCHWORK_FIX2019('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ORBITPATCHWORK_FIX2019.M with the given input arguments.
%
%      ORBITPATCHWORK_FIX2019('Property','Value',...) creates a new ORBITPATCHWORK_FIX2019 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before orbitPatchwork_fix2019_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to orbitPatchwork_fix2019_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help orbitPatchwork_fix2019

% Last Modified by GUIDE v2.5 05-Aug-2020 16:17:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @orbitPatchwork_fix2019_OpeningFcn, ...
                   'gui_OutputFcn',  @orbitPatchwork_fix2019_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before orbitPatchwork_fix2019 is made visible.
function orbitPatchwork_fix2019_OpeningFcn(hObject, eventdata, handles, varargin)
...
for II=1:numel(handles.XCOR.names)
    ...
    XC{II,6}=handles.XCOR.LIMITS(II,1);
    XC{II,7}=handles.XCOR.FAILED(II);
end
set(handles.XC,'data',XC);
set(handles.XC,'columnname',{'use','name','orig','MIN','new','MAX','failed'})
set(handles.XC,'ColumnEditable',[true,false,false,false,true,false,false]);
...
for II=1:numel(handles.YCOR.names)
    ...
    YC{II,6}=handles.YCOR.LIMITS(II,1);
    YC{II,7}=handles.YCOR.FAILED(II);
end
set(handles.YC,'data',YC);
set(handles.YC,'columnname',{'use','name','orig','MIN','new','MAX','failed'})
set(handles.YC,'ColumnEditable',[true,false,false,false,true,false,false]);
...
% Choose default command line output for orbitPatchwork_fix2019
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes orbitPatchwork_fix2019 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = orbitPatchwork_fix2019_OutputFcn(hObject, eventdata, handles) 
...
if(UpdateOnly)
   ...
   plot(handles.OY,static.zBPM(YBP([YB{:,1}])),ReadOutOrbit(YBP([YB{:,1}]),2),'--xr','linewidth',2);
    ...
    for II=1:size(XC,1)
        if(XC{II,1})
            XC{II,5}=Solution.NewCorr(Where_In_UsedCorr_X(II));
        end
    end
    for II=1:size(YC,1)
        if(YC{II,1})
            YC{II,5}=Solution.NewCorr(Where_In_UsedCorr_Y(II));
        end
    end
