function [q,dq,xf,yf,chisq,V] = plot_polyfit(x,y,dy,n,xtxt,ytxt,xunt,yunt,noplot,axisv,ll,res);
                                         
%       [q,dq,xf,yf,chisq,V] = plot_polyfit(x,y,dy,n,xtxt,ytxt[,xunt,yunt,noplot,axisv,ll,res]);
%                                   
...
%				V:		Covariance matrix (for propagating errors)
%
% P.Emma 11/13/91
...
if bars
  [yfit,dyfit,p1,dp1,chisq,V] = fit(Q,y,dy);
else                        
  [yfit,dyfit,p1,dp1,chisq,V] = fit(Q,y);
end
...
if noplot==0
  ...
  if ll<2
    for j = 1:mm
      text(xpn,ypn-j*0.040, ...
        [sprintf('p_%1.0f = %12.6e \\pm %12.6e',m(j),p(j),dp(j))],'Units','normalized')  
%        [sprintf('p_%1.0f = %7.4g+-%6.4g',m(j),p(j),dp(j))])  
    end
    text(0.10,0.030,[sprintf('RMS=%5.3g ',difbar) yunt],'Units','normalized')
    text(0.10,0.080,[sprintf('NDF=%5.0f ',NDF)],'Units','normalized')
    if bars
      text(0.02,0.030,[sprintf('chisq/N = %5.3f',chisq)],'Units','normalized')
    end
    if exist('r')
      text(0.02,0.080,[sprintf('rho     = %6.3f',r)],'Units','normalized')
    end
  end
end
