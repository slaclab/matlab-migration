function varargout = facet_posicomp(varargin)
...
% copied by Mario for e+'s 11/17/2014
% FYI: full complement PV = FCUDKLYS:MCC0:ONBC6SUMY
%
% facet_posicomp M-file for facet_posicomp.fig
%      facet_posicomp, by itself, creates gui and allows user to collect and
%      print current klystron complement to the facet physcis logbook
%
%      H = facet_posicomp returns the handle to a new facet_posicomp or the handle to
%      the existing singleton*.
%
%      facet_posicomp('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in facet_posicomp.M with the given input arguments.
%
%      facet_posicomp('Property','Value',...) creates a new facet_posicomp or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before facet_posicomp_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to facet_posicomp_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @facet_posicomp_OpeningFcn, ...
                   'gui_OutputFcn',  @facet_posicomp_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before facet_posicomp is made visible.
function facet_posicomp_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to facet_posicomp (see VARARGIN)
...
% Choose default command line output for facet_posicomp
...
function varargout = facet_posicomp_OutputFcn(hObject, eventdata, handles) 
...
function facet_posicomp_CloseRequestFcn(hObject, eventdata, handles)
...
klysmatrix = lcaGet('FCUDKLYS:MCC1:ONBC6SUMY');
...
try
    [arch_times,arch_data] = getHistoryWfm('FCUDKLYS:MCC1:ONBC6SUMY',timerange);
    [handles.time_index,handles.klys_stat] = bit_interp(arch_times,arch_data,timerange);
catch
    disp('Error retrieving archive waveform! Check status of PV: FCUDKLYS:MCC1:ONBC6SUMY')
