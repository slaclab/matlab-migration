...
% ================================================
data         = struct();
data.VersionInfo = struct('Version',0.4);
...
for scan_step=1:param.n_step
    ...
    for i=1:size(names,1)
        data.raw.scalars.(names{i})=add_raw(epics_data_mat(i,:),e_UID,descs{i},'EPICS');
    end
    ...
    if isfield(param,'scan2D_bool')
        if param.scan2D_bool
            ...
            data.raw.scalars.step_num_dim1 = add_raw(pv1_ind*ones(1,n_e_shots),e_UID,'Step Index 1','EPICS');
            data.raw.scalars.step_num_dim2 = add_raw(pv2_ind*ones(1,n_e_shots),e_UID,'Step Index 2','EPICS');
            
            data.raw.scalars.step_value_dim1 = add_raw(param.PV_scan_list1(pv1_ind)*ones(1,n_e_shots),e_UID,'Step Value 1','EPICS');
            data.raw.scalars.step_value_dim2 = add_raw(param.PV_scan_list2(pv2_ind)*ones(1,n_e_shots),e_UID,'Step Value 2','EPICS');
        end
    end
    ...
    % ================================================
    data.raw.scalars.step_num = add_raw(e_scan_step,e_UID,'Step Index','EPICS');
    data.raw.scalars.set_num  = add_raw(e_dataset, e_UID,'Set Number','EPICS');
    if scanbool
        data.raw.scalars.step_value = add_raw(param.PV_scan_list(scan_step)*ones(1,n_e_shots),e_UID,'Step Value','EPICS');
    end
...
for i=1:size(camstr,1)
    ...
    datafull.raw.images.(structstr)=replace_field(datafull.raw.images.(structstr),...
        'background_saved'	, cell_construct(param.save_back,1,n_i_shots));
...
% ================================================
datafull.raw.metadata.param = param;
datafull.raw.metadata.E200_state = E200_state;
