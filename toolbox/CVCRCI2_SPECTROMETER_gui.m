function varargout = CVCRCI2_SPECTROMETER_gui(varargin)
% CVCRCI2_SPECTROMETER_GUI MATLAB code for CVCRCI2_SPECTROMETER_gui.fig
%      CVCRCI2_SPECTROMETER_GUI, by itself, creates a new CVCRCI2_SPECTROMETER_GUI or raises the existing
%      singleton*.
%
%      H = CVCRCI2_SPECTROMETER_GUI returns the handle to a new CVCRCI2_SPECTROMETER_GUI or the handle to
%      the existing singleton*.
%
%      CVCRCI2_SPECTROMETER_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CVCRCI2_SPECTROMETER_GUI.M with the given input arguments.
%
%      CVCRCI2_SPECTROMETER_GUI('Property','Value',...) creates a new CVCRCI2_SPECTROMETER_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CVCRCI2_SPECTROMETER_gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CVCRCI2_SPECTROMETER_gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CVCRCI2_SPECTROMETER_gui

% Last Modified by GUIDE v2.5 23-Apr-2015 16:05:36

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @CVCRCI2_SPECTROMETER_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @CVCRCI2_SPECTROMETER_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CVCRCI2_SPECTROMETER_gui is made visible.
function CVCRCI2_SPECTROMETER_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CVCRCI2_SPECTROMETER_gui (see VARARGIN)

% Choose default command line output for CVCRCI2_SPECTROMETER_gui
...
% UIWAIT makes CVCRCI2_SPECTROMETER_gui wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CVCRCI2_SPECTROMETER_gui_OutputFcn(hObject, eventdata, handles) 
...
AS=get(handles.V_SEL,'string');
AV=get(handles.V_SEL,'value');
CurrentEnergy=lcaGetSmart('SIOC:SYS0:ML00:AO627');

currentspec=AS{AV};

if(~isempty(strfind(currentspec,'SXR:EXS:CVV:01:IMAGE_CMPX:HPrj')) || ~isempty(strfind(currentspec,'SXR:EXS:CVV:01:IMAGE_LIVE_FAST')))
    SpectrometerSetting.M1Pitch_SXR_MON_MMS_05=lcaGetSmart('SXR:MON:MMS:05.RBV');
    SpectrometerSetting.GratingPitch_SXR_MON_MMS_06=lcaGetSmart('SXR:MON:MMS:06.RBV');
    SpectrometerSetting.GratingTranslation_SXR_MON_MMS_07=lcaGetSmart('SXR:MON:MMS:07.RBV');
    
    if( SpectrometerSetting.GratingTranslation_SXR_MON_MMS_07 < 10)
       SpectrometerSetting.Linespermm=0; 
    elseif( SpectrometerSetting.GratingTranslation_SXR_MON_MMS_07 > 25)
       SpectrometerSetting.Linespermm=100;  
    else
       SpectrometerSetting.Linespermm=200;  
    end
    if(SpectrometerSetting.Linespermm==100)
        B12=CurrentEnergy;
        B18=1239.842;
        B21=3.7943;
        B22=-0.77294;
        ...
        B44=3.22E-05;
        B45=1.14E-17;
        B13=B33+B34*(B18/B12)+B35*(B18/B12)^2+B36*(B18/B12)^3+B37*(B18/B12)^4+B38*(B18/B12)^5;
        B14=(B42+B43*B12+B44*B12^2+B45*B12^3)/2.5;
        DISPeVPix=B14*(0.0043);
        set(handles.CalibCenter,'string',num2str(B12));
        set(handles.CalibLinear,'string',num2str(DISPeVPix));
        CalibLinear_Callback(hObject, eventdata, handles);
        CalibCenter_Callback(hObject, eventdata, handles);
        set(handles.MONOLOG,'string',['last auto calibration : ',datestr(clock),'   100 lines/mm, HOR axis in eV'])
    end
    
    if(SpectrometerSetting.Linespermm==200) 
        C12=CurrentEnergy;
    B18=1239.842;
    C21=1.8886;
    C22=-0.34777;
    ...
    C13=C33+C34*(B18/C12)+C35*(B18/C12)^2+C36*(B18/C12)^3+C37*(B18/C12)^4+C38*(B18/C12)^5;
    C14=(C42+C43*C12+C44*C12^2+C45*C12^3)/2.5/4;

        DISPeVPix=C14*(0.0043);
        set(handles.CalibCenter,'string',num2str(C12));
        set(handles.CalibLinear,'string',num2str(DISPeVPix));
        CalibLinear_Callback(hObject, eventdata, handles);
        CalibCenter_Callback(hObject, eventdata, handles);
        set(handles.MONOLOG,'string',['last auto calibration : ',datestr(clock),'   200 lines/mm, HOR axis in eV'])
    end
    
    if(SpectrometerSetting.Linespermm==0)
        set(handles.CalibCenter,'string','');
        set(handles.CalibLinear,'string','0.0043');
        CalibLinear_Callback(hObject, eventdata, handles);
        CalibCenter_Callback(hObject, eventdata, handles);
        set(handles.MONOLOG,'string',['last auto calibration : ',datestr(clock),'   0th order, HOR axis in mm'])
    end
    
    set(handles.MONOLOG,'userdata',SpectrometerSetting);
    return
end

if(~isempty(strfind(currentspec,'CAMR:FEE1:441:BLD1:HorizProj')))
    SpectrometerSetting.CALC_keV=lcaGetSmart('FEE:HXS.XENG');
    SpectrometerSetting.CALC_Crystal=lcaGetSmart('FEE:HXS.KRST');
    SpectrometerSetting.CALC_reflection=lcaGetSmart('FEE:HXS.S110');
    SpectrometerSetting.CALC_BraggAngle=lcaGetSmart('FEE:HXS.BANG');
    SpectrometerSetting.CALC_TopEnergyRange=lcaGetSmart('FEE:HXS.EMAX');
    SpectrometerSetting.CALC_BottomEnergyRange=lcaGetSmart('FEE:HXS.EMIN');
    SpectrometerSetting.CALC_CentralEnergy=mean([SpectrometerSetting.CALC_TopEnergyRange, SpectrometerSetting.CALC_BottomEnergyRange ]);
    SpectrometerSetting.CALC_Resolution=lcaGetSmart('FEE:HXS.DE');
    
    SpectrometerSetting.MotionCTRL_CrystalXmm=lcaGetSmart('STEP:FEE1:441:MOTR.VAL');
    SpectrometerSetting.MotionCTRL_CrystalAngleDeg=lcaGetSmart('STEP:FEE1:442:MOTR.VAL');
    SpectrometerSetting.MotionCTRL_CameraAngleDeg=lcaGetSmart('STEP:FEE1:443:MOTR.VAL');
    SpectrometerSetting.MotionCTRL_CameraXmm=lcaGetSmart('STEP:FEE1:447:MOTR.VAL');
    SpectrometerSetting.MotionCTRL_CameraDistancemm=lcaGetSmart('STEP:FEE1:444:MOTR.VAL');
    SpectrometerSetting.MotionCTRL_CameraIris=lcaGetSmart('STEP:FEE1:445:MOTR.VAL');
    SpectrometerSetting.MotionCTRL_CameraFilterSelection=lcaGetSmart('STEP:FEE1:446:MOTR.VAL');
    SpectrometerSetting.Transmission=lcaGetSmart('FEE:HXS:TRNS.SEVR');
    set(handles.CalibCenter,'string',num2str(SpectrometerSetting.CALC_CentralEnergy*1000));
    set(handles.CalibLinear,'string',num2str(-SpectrometerSetting.CALC_Resolution));
    CalibLinear_Callback(hObject, eventdata, handles);
    CalibCenter_Callback(hObject, eventdata, handles);
    set(handles.MONOLOG,'string',['last auto calibration: ',datestr(clock)])
    set(handles.MONOLOG,'userdata',SpectrometerSetting);
    return
end
...
petizione=get(handles.IdatiStanQua,'Userdata');
petizione.Write=get(handles.WritePVs,'value');
set(handles.IdatiStanQua,'Userdata',petizione);
