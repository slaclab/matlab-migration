function [q,dq,xf,yf] = gauss_plot(x,y,dy,y_off,no_txt,mrk)

%	[q,dq,xf,yf] = gauss_plot(x,y[,dy,y_off,no_txt,mrk])
%
...
%			the plot
%	mrk:		(Optional,DEF='o') Plot marker (e.g. 'o', 'x', '+',
%			'.', etc...)
%
...
if ~exist('dy')
  ...
  yyf = p(1)*ones(size(xx)) + p(2)*sqrt(2*pi)*p(4)*gauss(xx,p(3),p(4)); 
  plot(x,y,[mrk 'r'],xx,yyf,'-b')
  hold on
  plot_bars(x,y,dy,'.r','k')
  hold off
end
