function varargout = CVCRCI2_2DArray_vs_Scalar_gui(varargin)
% CVCRCI2_2DARRAY_VS_SCALAR_GUI MATLAB code for CVCRCI2_2DArray_vs_Scalar_gui.fig
%      CVCRCI2_2DARRAY_VS_SCALAR_GUI, by itself, creates a new CVCRCI2_2DARRAY_VS_SCALAR_GUI or raises the existing
%      singleton*.
%
%      H = CVCRCI2_2DARRAY_VS_SCALAR_GUI returns the handle to a new CVCRCI2_2DARRAY_VS_SCALAR_GUI or the handle to
%      the existing singleton*.
%
%      CVCRCI2_2DARRAY_VS_SCALAR_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CVCRCI2_2DARRAY_VS_SCALAR_GUI.M with the given input arguments.
%
%      CVCRCI2_2DARRAY_VS_SCALAR_GUI('Property','Value',...) creates a new CVCRCI2_2DARRAY_VS_SCALAR_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CVCRCI2_2DArray_vs_Scalar_gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CVCRCI2_2DArray_vs_Scalar_gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CVCRCI2_2DArray_vs_Scalar_gui

% Last Modified by GUIDE v2.5 21-Mar-2015 17:27:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @CVCRCI2_2DArray_vs_Scalar_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @CVCRCI2_2DArray_vs_Scalar_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CVCRCI2_2DArray_vs_Scalar_gui is made visible.
function CVCRCI2_2DArray_vs_Scalar_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CVCRCI2_2DArray_vs_Scalar_gui (see VARARGIN)

% Choose default command line output for CVCRCI2_2DArray_vs_Scalar_gui
...
petizione.PLAY=0;
set(handles.e_Condition,'userdata',0);
...
petizione.PartitionPos=NaN;
petizione.PartitionWidth=NaN;
...
% UIWAIT makes CVCRCI2_2DArray_vs_Scalar_gui wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CVCRCI2_2DArray_vs_Scalar_gui_OutputFcn(hObject, eventdata, handles) 
...
if(VAL==1)
   ...
   petizione.V_SEL=[VAL-1,SDA.Position_of_2Darrays_in_Profiles(VAL-1)];
end
set(handles.IdatiStanQua,'userdata',petizione);
...
if(~any(isnan(newval)))
    petizione.PartitionPos=newval;
end
set(handles.IdatiStanQua,'Userdata',petizione);
if(any(isnan(petizione.PartitionPos)) || any(isnan(petizione.PartitionWidth)))
    SETTINGSNUMBER=0;
else
    if(length(petizione.PartitionPos)==1)
       SETTINGSNUMBER=length(petizione.PartitionWidth);
    elseif(length(petizione.PartitionWidth)==1)
       SETTINGSNUMBER=length(petizione.PartitionPos); 
    elseif(length(petizione.PartitionWidth)==length(petizione.PartitionPos))
       SETTINGSNUMBER=length(petizione.PartitionPos);  
    else
       SETTINGSNUMBER=0;  
    end
...
set(handles.e_Condition,'userdata',SETTINGSNUMBER);
e_Condition_Callback(hObject, eventdata, handles);
...
if(any(isinf(newval))), newval=NaN; end
if(~any(isnan(newval)))
    petizione.PartitionWidth=newval;
end
set(handles.IdatiStanQua,'Userdata',petizione);
if(any(isnan(petizione.PartitionPos)) || any(isnan(petizione.PartitionWidth)))
    SETTINGSNUMBER=0;
...
function e_Condition_Callback(hObject, eventdata, handles)
MAX=get(handles.e_Condition,'userdata');
binsy=round(str2double(get(handles.e_Condition,'string')));
petizione=get(handles.IdatiStanQua,'Userdata');
if(isnan(binsy) || isinf(binsy) || (binsy>MAX) || (binsy<1))
    binsy=1;
end
set(handles.e_Condition,'string',num2str(binsy)); 
petizione.Condition=binsy;
set(handles.IdatiStanQua,'Userdata',petizione);

% --- Executes during object creation, after setting all properties.
function e_Condition_CreateFcn(hObject, eventdata, handles)
% hObject    handle to e_Condition (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function CalibLinearY_Callback(hObject, eventdata, handles)
petizione=get(handles.IdatiStanQua,'Userdata');
newval=str2double(get(handles.CalibLinearY,'string'));
if(isinf(newval)), newval=[]; set(handles.CalibLinearY,'string',''); end
if(isnan(newval)), newval=[]; set(handles.CalibLinearY,'string','');end
if(newval==0), newval=[]; set(handles.CalibLinearY,'string',''); end
petizione.calibY=newval;
set(handles.IdatiStanQua,'Userdata',petizione);


% --- Executes during object creation, after setting all properties.
function CalibLinearY_CreateFcn(hObject, eventdata, handles)
% hObject    handle to CalibLinearY (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function CalibCenterY_Callback(hObject, eventdata, handles)
petizione=get(handles.IdatiStanQua,'Userdata');
newval=str2double(get(handles.CalibCenterY,'string'));
if(isinf(newval)), newval=[]; set(handles.CalibCenterY,'string',''); end
if(isnan(newval)), newval=[]; set(handles.CalibCenterY,'string','');end
petizione.centerY=newval;
set(handles.IdatiStanQua,'Userdata',petizione);


% --- Executes during object creation, after setting all properties.
function CalibCenterY_CreateFcn(hObject, eventdata, handles)
% hObject    handle to CalibCenterY (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
petizione=get(handles.IdatiStanQua,'Userdata');
petizione.PLAY=get(handles.PLAY,'value');
set(handles.IdatiStanQua,'Userdata',petizione);
