function [ En_eV sEn_eV B0_T sB0_T M A sigR] = energybycorrector0(I_A,X_m,Y_m, PlotFlag)
%Perform the energy measurement using Corrector 0 (air coil) Horizontal and Screen 1.
%Sintax: [ En_eV sEn_eV B0_T sB0_T M A sigR] = energybycorrector0(I_A,X_m,Y_m, PlotFlag)
...
while ErrorFlg==0
    ...
    K=correctorfield(0,0,0,1); %Calculate the corrector calibration factor
    sK=0.1/sqrt(12)*K; % rms accuracy in corrector field calibration
    D=0.85;% Distance corrector 1 screen 1 in m
    %Calculate corrector equivalent lenght
    [trash1 trash2 trash3 L]=correctorproperties(0,0,1,1);
    ...
    title1='\fontsize{13}Hor. Corrector 0/Screen1 Beam Energy Measurement.';
    title2='\fontsize{6} ';
