function [AttnMotor,PhShiftMotor] = LbandHP_Attenuators_CalcMotorsForAttnWithPhaseConstant(AttnID, Attn_dB, PresentAttnMotorVal,PresentPhShftMotorVal)
% Calculate the motor settings for the high power L-band attenuator motors to apply 
% a given attenuation Attn_dB in dB and for the correspondent phase shifter to keep
% the phase constatnt
% AttnID=0 selects the T-Cav.
% [AttnMotor,PhShiftMotor] = LbandHP_Attenuators_CalcMotorsForAttnWithPhaseConstant(AttnID, Attn_dB)
...
AttMot0=PresentAttnMotorVal;% Attn motor initial value
...
for ii=1:Nsteps
    ...
    PhShiftMotor=LbandHP_PhaseShifters_CalcMotorForDeltaPhase(AttnID,PhStep,PresentPhShftMotorVal)
