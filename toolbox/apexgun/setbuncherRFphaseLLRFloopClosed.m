function [RFphase_out_RFdeg] = setbuncherRFphaseLLRFloopClosed(RFphase_RFdeg)
% Set the buncher RF phase when the LLRF phase/amp loop is closed.
% It also updates the phase value in the llrf_buncher.m Matlab GUI
% SINTAX: [RFphase_out_RFdeg] = setbuncherRFphaseLLRFloopClosed(RFphase_RFdeg)
...
setpvonline('L1llrf:set_iloop_re_ao',  Ref,'float',1);
setpvonline('L1llrf:set_iloop_im_ao',  Imf,'float',1);
...
setpvonline('L1llrf:source_re_ao',  Refgui,'float',1);
setpvonline('L1llrf:source_im_ao',  Imfgui,'float',1);
