...
% Last Modified by GUIDE v2.5 18-Sep-2015 14:01:10
...
%             '' '' '' '' ...
%             'OTRS:LI20:3070' 'OTRS:LI20:3175' 'PROF:LI20:3202' 'PROF:LI20:3208' ...
%             'OTRS:LI20:3208' 'PROF:LI20:3230' 'PROF:LI20:3483' 'PROF:LI20:3485' ...
%             '' '' '' '' ...
%             '' '' '' '' ...
%             '' '' '' '' ...
%             '' 'PROF:LI20:3484' ...
...
%             106:108 99:105 91:97];
    ...
    otherwise
        ...
        handles.profmonMap=[0 102:110 101 63 64 65 1 95:100 67 66 2:5 21 20 22 7:12 14:16 44 ...
            23 83 84:86 49 25 111 26 30 61:62 28:29 31 112 32:36 68 87 88 50:51 ...
...
switch accelerator
    ...
    otherwise
        handles.wireList={ ...
            ...
            'WIRE:LTUH:246'  'WIRE:DMP1:696'  'WIRE:LTUH:122' 'WIRE:LTUH:538'};
        handles.wireMap= [0:11,16, 18, 19, 12:15];
end
handles.wireId=0;
handles.wireName='';
...
switch accelerator
    ...
    case 'FACET'
        ...
        handles.emitMap=[0 11:14 8:9 15:17];
    otherwise
...
switch accelerator
    ...
    otherwise
        ...
        handles.blenMap=[0 1 8 2 5 4 6 7 9];
end
handles.blenId=0;
handles.blenName='';
...
handles.configList={'ctrlPVNum' 'ctrlPVName' 'ctrlMKBName' 'ctrlPVValNum' 'ctrlPVRange' 'ctrlPVWait' ...
    'ctrlPVWaitInit' 'settlePVName' 'readPVNameList' 'plotHeader' 'acquireSampleNum' ...
...
handles.settlePVName='';
handles.readPVNameList={'BPMS:IN20:221:TMIT';'GDET:FEE1:241:ENRC'};
...
handles=settlePVControl(hObject,handles,[]);
...
% ------------------------------------------------------------------------
function handles = settlePVControl(hObject, handles, val)

handles=gui_textControl(hObject,handles,'settlePVName',val);
str={'Initial Settle' 'Settle Condition'};
set(handles.ctrlPVWaitInitLabel_txt,'String',str{~isempty(handles.settlePVName)+1});
...
typ='scan';if handles.emitId && ismember(handles.emitName,{'WIRE:LI28:144' 'WIRE:LTUH:735'}), typ='multi';end
...
pvSet(pv,val);
if ~isempty(handles.settlePVName)
    strS=sprintf('Waiting for %s',char(handles.settlePVName));disp(strS);
    pause(.5);timeOut=now;
    while lcaGetSmart(handles.settlePVName,0,'double') ~= handles.ctrlPVWaitInit
        pause(.5);if now > timeOut+1/24/60*5, disp('Timeout occured');break, end
        % 5 min timeout
    end
else
...
camName = handles.data.dataList(iVal,1,1).name;
...
for j=iVal
    for k=1:size(handles.data.dataList(j,:,:),2)
        for l=1:size(handles.data.dataList(j,:,:),3)
            ...
            if doPut
                try
                    control_profDataSet(camName, data.beam);
                catch ex
                    disp('Error trying to post processed camera stats!')
                    disp(ex.message)
                end
            end
...
if any(handles.profmonId) && ~handles.profmonBSA
    ...
    opts.axes=handles.plotProf_ax;opts.doPlot=1;opts.doProcess=0;
...
if ~all(handles.BSAPVValid)
    ...
    gui_statusDisp(handles,sprintf('Getting Samples #1-%d',handles.dataSample.nVal));
...
if handles.blenId && isfield(data,'blenBeam')
    plane='y';
    if isfield(data,'blenPV') && ...
            strcmp(data.blenPV(1).name(1:13), 'OTRS:DMP1:695')
        plane = 'x';
    end
end
...
% Store plot results.
%handles.data.(xValList,yValList,xStdList,yStdList,xFit,yFit,yFitStd,par,parstd,strfit)
guidata(hObject,handles);
...
switch handles.show2D
    case 1
        ...
        use=logical(handles.data.status);
        [x,u,y]=deal(nan(siz));
        x(use)=xPVValMean;u(use)=uPVValMean;y(use)=yPVValMean;
        surf(x,u,y);
        shading interp
    case 2
...
if handles.useStaticBG && ~isempty(handles.profmonName)
    if handles.profmonNumBG
        ...
        opts.nAvg=handles.profmonNumBG;
...
function settlePVName_txt_Callback(hObject, eventdata, handles)

settlePVControl(hObject,handles,get(hObject,'String'));
