function [sortedStruct index] = util_nestedSortStruct(aStruct, fieldNamesCell, directions)
...
% nestedSortStruct will simply call util_sortStruct. This will be faster than putting the
...
% Retrieved from Matlab File Exchange Jun 2014
% Author: Jake Hughey
% Added: Sep 29, 2016
...
if ~iscell(fieldNamesCell)
    if isfield(aStruct, fieldNamesCell) % if fieldNamesCell is a simple string of a valid fieldname
        if nargin < 3
            [sortedStruct index] = util_sortStruct(aStruct, fieldNamesCell);
        else
            [sortedStruct index] = util_sortStruct(aStruct, fieldNamesCell,directions);
        end
        return
    else
...
%workaround for OLD MATLABS... need to strip out stuff that aren't numbers
%to use sortrows.

bCell = cell(size(aCell,1),length(fieldNamesIdx));
for k = 1:length(fieldNamesIdx)
    bCell(:,k) = aCell(:,fieldNamesIdx(k));
end
[sortedCell index] = sortrows(cell2mat(bCell), (1:length(fieldNamesIdx)) .* directions);
sortedStruct = aStruct(index); % apply the index to the struct array