function [sz,sd,k,Ipk,dE_E,dt,Eloss,sdsgn] = double_compress(sz0,sd0,Eg,Ev,R56v,T566v,phiv,Lv,N,...
                                                             dN_N,dtg,dphiv,dV_Vv,lamv,s0v,av,gu);

% [sz,sd,k,Ipk,dE_E,dt,Eloss,sdsgn] = double_compress(sz0,sd0,Eg,Ev,R56v,T566v,phiv,Lv,N,...
%                                                     dN_N,dtg,dphiv,dV_Vv,lamv,s0v,av,gu);
%
...
%                   gu:     (Optional, DEF='u') Flag for Ipk from Gaussian
%                           (gu=='g') or Uniform dist. (gu='u')
%
...
%if Nv ~= 5
%  error('This function requires five energy values, etc. - quitting.')
%end
%if (length(R56v)~=Nv) | (length(T566v)~=Nv) | (length(phiv)~=Nv) |  (length(Lv)~=Nv)
%  error('The vectors "R56v", "T566v", "phiv" and "Lv" need to be the same length as "Ev" - quitting.')
%end
%if any(Ev<0)
%  error('Energy values are always >0 - quitting.')
%end
%if any(T566v<0)
%  error('T566 values are always >0 - quitting.')
%end

%if ~exist('dN_N')		% set default values...
%  dN_N = 0;
%end
%if ~exist('dtg')
%  dtg = 0;
...
Ipk   = (1+dN_Nf)*N*e*c/Ipk_fac./sz;			    % calculate peak current over uniform bunch dist. [A]
sz    = sz*1E3;						            % m -> mm
sd    = sd*1E2;						            % [ ] -> %
