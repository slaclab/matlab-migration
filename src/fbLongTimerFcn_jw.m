...
% Let us force 24-1 phase is always larger than 24-2 phase --jw
if str2num(loop.feedbackNum)>3
    len = loop.act.numactPVs;
    actpos = max(newact(len,1),newact(len - 1,1));
    actneg = min(newact(len,1),newact(len - 1,1));
    newact(len - 1,1) = actpos;
    newact(len,1)     = actneg;
end
% end of modification for polarization --jw
