function [kName,kStat,kAmpl,kPhas,kEerr,kFudge,kEGain,kPower,Eref,oldFudge,newFudge]=LEMLite(fFlag)
%
% [kName,kStat,kAmpl,kPhas,kEerr,kFudge,kEGain,kPower,Eref,oldFudge,newFudge]=LEMLite(fFlag);
%
% INPUT:
%
%   fFlag=0 : use "current" fudge factors
%   fFlag=1 : use "ideal" fudge factors [default]
%
% OUTPUT arrays (for each KLYS unit):
%
%   kName    = XAL root name
%   kStat    = trigger status for beam code 1 (0=OFF,1=ON)
%   kAmpl    = on-crest no-load energy gain (MeV) ... DB value times kEerr
%   kPhas    = RF phase values [KLYS,SBST] (degrees)
%   kEerr    = static multiplicative fudge factor
%   kFudge   = LEM fudge factor
%   kEGain   = energy gain (GeV)
%   kPower   = power factors for [a,b,c,d] sections (E ~ sqrt(P))
%
% OTHER OUTPUTs:
%
%   Eref     = reference energy values (GeV)
%   oldFudge = current fudge factors
%   newFudge = ideal fudge factors

% ------------------------------------------------------------------------------
% 21-JAN-2009, M. Woodley
%    Solve quadratic equation for fudge factor explicitly; add more checks for
%    "unphysical" fudge factor values ... default to 1.0 if a real positive
%    value isn't found
...
% twoPhase flag (if set, include SLC KLYS phases for globally phased units)

twoPhase=0;

% input argument and default
...
% get current fudge factors and reference energy values
...
klysList=LEMLiteKlysInfo();
klysCount=length(klysList);
klysStat=zeros(klysCount,1);
klysAmpl=zeros(klysCount,1);
klysPhas=zeros(klysCount,2);
klysEerr=[klysList.Eerr]';

for n=1:klysCount
  name=klysList(n).xalNameRoot;
  ctrl=klysList(n).ctrlFlags;
...
%----
  ...
  klysAmpl(n)=ampl*klysEerr(n);
  klysPhas(n,1)=phas;
  if (ctrl(3)~=0)
    klysPhas(n,2)=sphas(ctrl(3));
  end
end

% compute ideal fudge factors

Phas=deg2rad*(klysPhas(:,1)+klysPhas(:,2)); % aggregate phase (rad)
kdE=MeV2GeV*(klysStat.*klysAmpl.*cos(Phas)); % energy contribution (GeV)
...
kName=cell(klysCount,1);
kStat=klysStat;
kAmpl=klysAmpl;
kPhas=klysPhas;
kEerr=klysEerr;
kFudge=zeros(klysCount,1);
kEGain=zeros(klysCount,1);
kPower=zeros(klysCount,4);

for n=1:klysCount
  kName{n}=klysList(n).xalNameRoot;
  m=length(find(n>idref));
  kFudge(n)=Fudge(m);
  rawEGain=MeV2GeV*kStat(n)*kAmpl(n)*cos(Phas(n)); % GeV
  if (rawEGain<0)
    kEGain(n)=rawEGain/kFudge(n);
  else
    kEGain(n)=rawEGain*kFudge(n);
  end
  kPower(n,:)=klysList(n).powerFact;
end
kName=char(kName);

end

function Klys=LEMLiteKlysInfo()
