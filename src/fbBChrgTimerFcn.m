...
if ((wpVals(1) ==1) && ((wpVals(2) ==0) && (wpVals(3)==1)))
      % both waveplates stopped and ready for another command
      calcFeedback;
else
   if ((wpVals(1) ==0) || ((wpVals(2) ==0) && (wpVals(3)==0)))
...
try
   ...
   curr_meas = curr_meas(end-loop.samples:end);
   checkOK = 1;
catch
   fbDispMsg(['CA error reading ' loop.meas.chosenmeasPVs],loop.feedbackAcro, 2);
   disp( ['CA error reading ' loop.meas.chosenmeasPVs] );
   %update the loop structure and store it
   setappdata(0,'Loop_structure',loop);
   return;
end
