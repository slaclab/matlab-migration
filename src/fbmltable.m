function data = fbmltable(fig, hObj, action, columnInfo, rowHeight, cell_data, gFont, varargin)
% function data = fbmltable(fig, hObj, action, columnInfo, rowHeight, cell_data, gFont)
%
...
% 
% fbmltable(fig, tbl, 'CreateTable', columninfo, rowHeight, cell_data, gFont);
%
...
% OpeningFcn, but replace the fbmltable line with:
%
% fbmltable(gcf, handles.tblParams, 'CreateTable', columninfo, rowHeight, cell_data, gFont);
% so clicking outside the table will finish edit in progress...
% endfcn = sprintf('fbmltable(%14.13f, %14.13f, ''SetCellValue'');', hObject, handles.tblParams);
...
% If necessary to call from figure use: fbmltable(fig, hObj, 'Action',...)
...
editfcn = sprintf('fbmltable(%14.13f, %14.13f, ''EditCell'');',fig, hObj);
set(data.axes, 'ButtonDownFcn', editfcn);
% callback for scrolling table
scrfcn = sprintf('fbmltable(%14.13f, %14.13f, ''ScrollData'');',fig, hObj);
...
if sum(columnInfo.isEditable) > 0 && (~isfield(columnInfo,'rowsFixed') ||...
	...
	btnfcn = sprintf('fbmltable(%14.13f, %14.13f, ''AddRow'');',fig, hObj);
	...
	btnfcn = sprintf('fbmltable(%14.13f, %14.13f, ''DelRow'');',fig, hObj);
...
% so clicking outside the table will finish edit in progress
endfcn = sprintf('fbmltable(%14.13f, %14.13f, ''SetCellValue'');', fig, hObj);
...
for j = 1:data.numRows
	...
	if data.doCheck % put checkboxes to left of row
		...
		chkfcn = sprintf('fbmltable(%14.13f, %14.13f,''OnCheck'',[], [], [], [], %d);',...
          fig, hObj, j);
...
if data.columnInfo.isNumeric(col)
    ...
    if fbmltable(fig, hObj, 'OnSetCell', [], [], [], [], d_old, d_new(1))
    ...
    if fbmltable(fig, hObj, 'OnSetCell', [], [], [], [], data.data{row, col}, nums)
...
% call as "fbmltable(fig, hobj, 'SetDblClick', 'Myfunc', 'fmt_str', args..."
...
% ------------------------------------------------------------------------
	...
	data = fbmltable(fig, tbl, 'CreateTable', columninfo, rowHeight, cell_data, gFont);
return
