function roi = imgProcessing_rubberbandRawImg(figName, img, ipParam, oldCropArea)
if nargin < 4
    oldCropArea = [];
end

cropFig = imgUtil_gui_detachedLiveImg();
set(cropFig, 'name', figName);
imgUtil_setFigColormap(cropFig, ipParam);
gh = guihandles(cropFig);

ipOutput = imgData_construct_ipOutput();
ipOutput.procImg = img;

imgProcessing_showProcessedImg(gh.imgAxes, imgData_construct_ipParam(), ipOutput);
if ~isempty(oldCropArea)
    imgProcessing_annotation_showCropArea(gh.imgAxes, oldCropArea);
end

try
    roi = getrect(gh.imgAxes);
    close(cropFig);
catch
    roi = [];
end
...
%   $Revision: 1.6 $  $Date: 2008/02/06 20:09:32 $
