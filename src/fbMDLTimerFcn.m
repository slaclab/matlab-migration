function fbMDLTimerFcn(obj, event)
% --------- function ----------------------------------------------
...
% which can update slowly. This can be removed once Linac Upgrade complete.
tmo=lcaGetTimeout;
    ...
    for i=1:loop.meas.nummeasAvg
        try
            ...
            elseif strfind(err.identifier,'invalidArg')
                % This identifier may indicate some measurement PVs
                % offline
        ...
        if any(isnan(curr_meas))
            ...
            exception = MException('fbMDLTimerFcn:MeasNaN', 'Measurement(s) returned NaN');
            throw(exception);
        end      
        % If SLCCAS PV, check .STAT for error condition. This can be
        % removed once Linac Upgrade complete.
        %
        % Get sector number. 
        % Match sector number with list of upgrade sectors. 
        %   If no match, assume is EPICS PV and skip this section.
        %   If there is a match, use the saved data from fb init to determine 
        %   if Micro or IOC controlled. 
        %     If Micro, get .STAT. Test...
        %{
        for j=1:length(loop.meas.chosenmeasPVs)
            s=fbGetPvSectorNumber(loop.meas.chosenmeasPVs(j));
            if ~strcmp(s,'') 
                s=sprintf('LI%s',char(s));
                r=strmatch(s,loop.act.sectors);
                if r
                    c = loop.act.camac(r);
                    if c == 0
                        ...
                        if st > 1
                            ...
                            exception = MException('fbMDLTimerFcn:MeasBadStat', 'Measurement(s) bad status');
                            throw(exception);
                        end
                    end
                end
...
% Restore lca timeout. This can be removed once Linac Upgrade complete.
lcaSetTimeout(tmo);
            ...
            if (newactsok == 0 ) % If successful
                ...
                if (actsok == 0) % If successful
                    ...
                    if (actslim == 0) % If actuator settings within limits
                        ...
                        % Set actuators
                        try
                            put_lca_aida(loop.act.chosenactPVs, new_act, loop.act.sectors, loop.act.camac);
                ...
                try
                    ...
                    message = 'Cannot save feedback data to FB00';
                    fbLogMsg(sprintf('MDL: %s',message));
                    fbDispMsg(message, loop.feedbackAcro, 2);
                    disp(message);
                end
...
%   They are hard-coded in fbInitMDLLoop.m.
%
%   Changes from SLC loop: 
%   - Instead of using Sector 20 PMDL as reference to calculate CMND_OLD, 
%   use Sector 30 (or rather, the last sector of the chosen actuators).
%     Thus CMND_OLD formula changes to:
%     CMND_OLD=-PMDL_LI30/((sec_last-sec_zero)*CONSTANT) 
%     sec_last and sec_zero are defined in fbInitMDLLoop.m.
...
try  
    ...
    if strfind(err.identifier,'timedOut')
        % PV get failed
        message='NO change to acts. LI30 PMDL get failed.';
...
%
%   We only implement correction for sectors LI25 and beyond.
%
%   Loop through actuator PVs, get sector number for each PV. If sector
%   number less than 25, set new act value to current act value (i.e. no
%   change), else set to to calculated value.
%
%   Arguments:
%
%           loop        Global loop data structure
...
try
    ...
    new_act=zeros(n,1);
    sectors=str2double(fbGetPvSectorNumber(loop.act.chosenactPVs));
    for i=1:n
        if sectors(i) < 25
            new_act(i) = loop.actData.current + 0;
        else
            new_act(i) = -(sectors(i) - 21)*loop.misc.constant*cmnd;
        end
    end
catch
    err=lasterror;
