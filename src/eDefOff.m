...
if (eDefNumber == OneHertz) || (eDefNumber == TenHertz) || (eDefNumber == BeamRate)
    ...
    put2log(sprintf('Sorry %s, eDefOff(%d) prohibited.', caller, eDefNumber));
    ...
    if ~iscell(reservedBy) && isnan(reservedBy)
        ...
        put2log(sprintf('Sorry %s, eDefOff can''t find pv %s', caller, pv));

    elseif isempty(char(reservedBy))

        put2log(sprintf('Sorry %s, eDefOff(%d) app name is empty', caller, eDefNumber));

    else

        if isempty(whos('global','eDefQuiet'))
            put2log(sprintf('%s about to eDefOff(%d) reserved by ''%s''',caller, eDefNumber, char(reservedBy)));
        end
