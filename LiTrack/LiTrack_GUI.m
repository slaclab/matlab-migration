...
% Last Modified by GUIDE v2.5 03-Mar-2016 15:44:32
...
if nargin == 3,
  if exist('./Wakefiles','dir') 
    wake_dir = [ pwd filesep 'Wakefiles' ];
  else
    wake_dir = pwd;
  end
  if exist('./SaveFiles','dir') 
    save_dir = [ pwd filesep 'SaveFiles' ];
  else
    save_dir = pwd;
  end
  if exist('./ParticleFiles','dir') 
    file_dir = [ pwd filesep 'ParticleFiles' ];
  else
      file_dir = pwd;
  end
%   wake_dir = pwd;
...
function clear_beamline( handles )
...
switch x
  ...
  case 15
    ...
    fontsize = 9;
  case 16
    str = '16:Dechirp radius/m   L/m     per/m   gap/m   depth/m';
    fontname = 'Courier';
    fontsize = 9;
  case 17
...
function updateBeamline( handles )
% Update beamline panel and related GUI handles from prepared data.
...
% Update beamline array display panel
for j = 1:25
    cmnd = ['set(handles.radiobutton' int2str(j) ',''Value'',' int2str(handles.inp_struc.p(j)) ');'];
    eval(cmnd)
    code = abs(handles.inp_struc.beamline(j,1));
...
% --- Executes on button press in designinit.
function designinit_Callback(hObject, eventdata, handles)
% hObject    handle to designinit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%
% Call lcls longitudinal data init with name of particle file if
% one has been selected.

% TODO: Check if prepData sets anything other than beamline. If so,
%       initialization dialog should be outside beamline panel.

LiTrack_globals;   % DESIGN etc.
LiTrack_messages;  % LiTrack message codes and text.

try
    % If the user has selected particle profile from file, ie Int(ernal)
    % == 0, then extract it form inp_struc and pass it as named arg
    % 
    if ( handles.inp_struc.Int == 0 ) % Int == 0 use particle file
        if ( isfield(handles.inp_struc,'zd_file') )
            zd_file_ = handles.inp_struc.zd_file;
            inp_struc_ = LiTrack_prepData_LCLS(DESIGN,'ParticleFn',zd_file_);
        else
            error(LIT_IE_INPFROMEXTBUTNOZDFIELD, LIT_IE_INPFROMEXTBUTNOZDFIELDMSG);
        end
    else
        inp_struc_ = LiTrack_prepData_LCLS(DESIGN);
    end
    handles.inp_struc = inp_struc_;
    
    % Update beamline panel from new data, and update status msg
    updateBeamline( handles );
    set(handles.MSGBOX,'String','Data matrix initialized for design LCLS');
    
catch ME
    
    message = sprintf('Could not set beamline to LCLS design. %s', ME.message);
...
% --- Executes on button press in extantinit.
function extantinit_Callback(hObject, eventdata, handles)
% hObject    handle to extantinit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% Call lcls longitudinal data finder with name of particle file if
% one has been selected.
try
    % If the user has selected particle profile from file, ie Int(ernal)
    % == 0, then extract it form inp_struc and pass it as named arg
    ...
    if ( handles.inp_struc.Int == 0 ) % Int == 0 use particle file
        if ( isfield(handles.inp_struc,'zd_file') )
            zd_file_ = handles.inp_struc.zd_file;
            inp_struc_ = LiTrack_prepData_LCLS(EXTANT,'ParticleFn',zd_file_);
        else
            error(LIT_IE_INPFROMEXTBUTNOZDFIELD, LIT_IE_INPFROMEXTBUTNOZDFIELDMSG);
        end
    else
        inp_struc_ = LiTrack_prepData_LCLS(EXTANT);
    end
    handles.inp_struc = inp_struc_;
    
    % Update beamline panel from new data, and update status msg
    updateBeamline( handles );
    set(handles.MSGBOX,'String','Data matrix initialized for extant LCLS.');
    
catch ME
    
    message = sprintf('Could not set beamline description from extant LCLS. %s', ...
        ME.message);
    uiwait(errordlg(message));
